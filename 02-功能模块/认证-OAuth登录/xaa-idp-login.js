...
import { renderOAuthCallbackPage, getFirstParamValue, pickOAuthCallbackPort } from "../../01-核心基础设施/共享小工具-未细化/oauth-callback.js";
...
async function getCachedIdpIdToken(n) {
  let r = (await getSecureStorage().readAsync())?.mcpXaaIdp?.[issuerKey(n)];
  if (!r) return;
...
async function k(n, t, e) {
  await getSecureStorage().mutate((r) => ({
    ...r,
    mcpXaaIdp: { ...r.mcpXaaIdp, [issuerKey(n)]: { idToken: t, expiresAt: e } },
  }));
}
async function saveIdpIdTokenFromJwt(n, t) {
  let e = C(t),
    r = e ? e * 1000 : Date.now() + 3600000;
  return (await k(n, t, r), r);
}
async function clearIdpIdToken(n) {
  let t = issuerKey(n);
  try {
...
async function saveIdpClientSecret(n, t) {
  try {
    return await getSecureStorage().mutate((e) => ({
      ...e,
      mcpXaaIdpConfig: { ...e.mcpXaaIdpConfig, [issuerKey(n)]: { clientSecret: t } },
    }));
  } catch (e) {
    return { success: !1, warning: l(e) };
  }
}
async function getIdpClientSecret(n) {
  return (await getSecureStorage().readAsync())?.mcpXaaIdpConfig?.[issuerKey(n)]?.clientSecret;
}
async function clearIdpClientSecret(n) {
  let t = issuerKey(n);
  try {
    await getSecureStorage().mutate((e) => {
...
function A(n, t) {
  ...
  }).catch((e) => rethrowFetchError(e, n));
}
async function discoverOidc(n) {
  let t = n.endsWith("/") ? n : n + "/";
  if (!URL.canParse(".well-known/openid-configuration", t))
    throw new R(
      `XAA IdP: OIDC discovery failed: issuer is not a valid URL: ${redactUrl(n)}`,
  ...
  if (!r.ok)
    throw Error(
      `XAA IdP: OIDC discovery failed: HTTP ${r.status} at ${redactUrl(e.href)}`,
    );
  let i;
  try {
    i = await r.json();
  ...
  if (
    ...
    throw Error(
      `XAA IdP: refusing non-HTTPS token endpoint: ${redactUrl(a.data.token_endpoint)}`,
    );
  return a.data;
}
function C(n) {
  ...
  return new Promise((m, g) => {
    ...
    ((i = createServer((s, o) => {
      ...
      let y = u.query.code,
        S = u.query.state,
        h = u.query.error,
        b = getFirstParamValue(u.query.iss);
      if (h) {
...
async function acquireIdpIdToken(n) {
  return withFeatureTelemetry("mcp_xaa_idp_login", async () => {
    let { idpIssuer: t, idpClientId: e } = n,
      r = await getCachedIdpIdToken(t);
    if (r) return (logMCPDebug("xaa", `Using cached id_token for ${redactUrl(t)}`), r);
    logMCPDebug("xaa", `No cached id_token for ${redactUrl(t)}; starting OIDC login`);
    let i = await discoverOidc(t),
      a = n.callbackPort ?? (await pickOAuthCallbackPort()),
    ...
    try {
      (await k(t, d.id_token, o),
        logMCPDebug(
          "xaa",
          `Cached id_token for ${redactUrl(t)} (expires ${new Date(o).toISOString()})`,
        ));
    } catch (u) {
