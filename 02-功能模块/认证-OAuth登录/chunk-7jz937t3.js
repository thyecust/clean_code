...
function Lct(t) {
  ...
  for (let [o, r] of t.searchParams) {
    let c = x(o, r !== "");
    e.append(c, c === "REDACTED" ? "REDACTED" : Zw(o, r));
  }
  let n = e.toString();
  return n === "" ? "" : `?${n}`;
}
function Zw(t, e) {
  ...
  switch (n) {
    case "sensitive":
      return "[REDACTED]";
    case "url":
      return A(e) || A(e.replaceAll(" ", "+")) ? Gn(e) : "REDACTED";
    case "judged":
      return st(e);
  }
}
...
function at(t, e) {
  let n = String(e);
  return y(n) && !D(t)
    ? `${t instanceof Error ? t.name : typeof t} fetching ${Gn(n)}`
    : l(t);
}
function ct(t) {
  try {
    return Gn(t) === new URL(t).href;
  } catch {
    return !1;
  }
}
...
function H(t, e, n) {
  ...
  let o = t instanceof Error ? /\bHTTP (\d{3})\b/.exec(t.message)?.[1] : void 0,
    r = tf(t)?.code;
  return `${o ? `HTTP ${o}` : t instanceof Error ? t.name : typeof t} ${n} ${Gn(e)}${r ? ` (${r})` : ""}`;
}
function O_(t) {
  if (D(t)) return l(t);
  let e = tf(t)?.code;
...
export { g9, Lct, Zw, Gn, _E, O_, as, w7, yE };
