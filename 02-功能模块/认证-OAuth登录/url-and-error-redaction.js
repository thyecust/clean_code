...
function p(t) {
  return (
    /[a-z]/i.test(t) && (/\d/.test(t) || (/[a-z]/.test(t) && /[A-Z]/.test(t)))
  );
}
function h(t) {
...
var K =
  ...
  Z =
    /^(?:gzip|x-gzip|deflate|br|zstd|compress|identity|\*)(?:\s*;\s*q=[\d.]+)?$/i,
  q = /^charset=[\w.:-]+$/i,
  G = /(?<![;,\s])[;,\s]+$/;
function L(t) {
  let e = t
...
function rt(t, e) {
  ...
  return v.test(e) || (L(e) && !nt(e));
}
function redactHeaders(t) {
  let e = {};
  for (let [n, o] of Object.entries(t)) {
    let r = n.toLowerCase();
    e[n] = V.has(r) && rt(r, o) ? o : "[REDACTED]";
  }
  return e;
}
function P(t, e = !1) {
  ...
  for (let r of n.matchAll(J)) {
    let c = r.index + r[0].length;
    if (n[c] !== ":" || !/[a-z]/i.test(r[0])) continue;
    if (/[/\\]/.test(n[c + 1] ?? "") || (!e && X.test(r[0])) || o > c)
      return !0;
  }
  return !1;
}
...
function redactSearchParams(t) {
  let e = new URLSearchParams();
  for (let [o, r] of t.searchParams) {
    let c = x(o, r !== "");
    e.append(c, c === "REDACTED" ? "REDACTED" : redactParamValue(o, r));
  }
  let n = e.toString();
  return n === "" ? "" : `?${n}`;
}
function redactParamValue(t, e) {
  let n = ot.get(t);
  ...
  switch (n) {
    case "sensitive":
      return "[REDACTED]";
    case "url":
      return A(e) || A(e.replaceAll(" ", "+")) ? redactUrl(e) : "REDACTED";
    case "judged":
      return st(e);
  }
}
function st(t) {
  let e = [],
    n = [],
    o = () => {
      if (n.length > 0) (e.push(...it(n)), (n = []));
    };
  for (let r of t.split(" "))
    if (A(r)) (o(), e.push(redactUrl(r)));
    else n.push(r);
  return (o(), e.join(" "));
}
function it(t) {
    ...
    o = () => {
      if (n.length > 0)
        (e.push(
          ...(d(n.join("+"))
            ? ["REDACTED"]
            : n.map((r) => (d(r) ? "REDACTED" : r))),
        ),
          (n = []));
    };
  for (let r of t)
    if (r === "" || S.test(r)) n.push(r);
    else (o(), e.push(d(r) || T(r) ? "REDACTED" : r));
  return (o(), e);
}
function redactUrl(t) {
  let e;
  try {
    e = new URL(t);
  } catch {
  ...
  let o = (s) => {
    ...
    c = (s) => {
      let i = r(s);
      if (A(i) || T(i)) return "REDACTED";
      let a = i;
      for (let g = 0; g < 3; g++) {
        let u = E(a);
        if (u === a) break;
        if (r(u) !== u || _(u) !== -1 || T(u)) return "REDACTED";
        a = u;
      }
      return i;
    },
    C = `${n ? e.protocol : ""}${e.pathname}`,
    M = _(C),
    f;
  if (P(e.pathname, !0)) {
    ...
    if (M !== -1) {
      let i = (U) => countOccurrences(C.slice(0, U), "/"),
        a = i(C.indexOf(":")),
        g = i(M),
        u = s[g] ?? "",
...
function D(t) {
  return (
    ...
    (t instanceof Error &&
      (t.name === "TimeoutError" ||
        t.name === "AuthenticationCancelledError")) ||
    t instanceof R
  );
}
function at(t, e) {
  let n = String(e);
  return y(n) && !D(t)
    ? `${t instanceof Error ? t.name : typeof t} fetching ${redactUrl(n)}`
    : l(t);
}
function ct(t) {
  try {
    return redactUrl(t) === new URL(t).href;
  } catch {
    return !1;
  }
}
function formatMcpSdkError(t, e) {
  return H(t, String(e), "from the MCP SDK for");
}
function H(t, e, n) {
  if (D(t) || ct(e)) return l(t);
  let o = t instanceof Error ? /\bHTTP (\d{3})\b/.exec(t.message)?.[1] : void 0,
    r = tf(t)?.code;
  return `${o ? `HTTP ${o}` : t instanceof Error ? t.name : typeof t} ${n} ${redactUrl(e)}${r ? ` (${r})` : ""}`;
}
function formatErrorWithCode(t) {
  if (D(t)) return l(t);
  let e = tf(t)?.code;
  return `${t instanceof Error ? t.name : typeof t}${e ? ` (${e})` : ""}`;
}
function formatConnectionError(t, e) {
  switch (e.type) {
    case "ws":
    case "ws-ide":
    case "sse":
...
function redactErrorForLogging(t, e) {
  let n = formatConnectionError(t, e);
  return n === l(t) ? t : n;
}
function rethrowFetchError(t, e) {
  if (D(t)) throw t;
  if (y(String(e))) {
    let n = tf(t)?.code;
    throw Object.assign(TypeError(at(t, e)), n === void 0 ? {} : { code: n });
  }
  throw t;
}
export { redactHeaders, redactSearchParams, redactParamValue, redactUrl, formatMcpSdkError, formatErrorWithCode, formatConnectionError, redactErrorForLogging, rethrowFetchError };
