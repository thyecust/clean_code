...
async function X(e) {
  ...
  try {
    let o = OAuthErrorResponseSchema.parse(JSON.parse(r)),
      { error: n, error_description: i, error_uri: u } = o;
    return new (E[n] || ServerError)(i || "", u);
  } catch (o) {
...
async function auth(e, t) {
  try {
    ...
    else if (r instanceof InvalidGrantError)
...
async function H(
  ...
  if (u?.authorizationServerUrl) {
    if (
      ...
      (p = u.authorizationServerMetadata ?? (await discoverAuthorizationServerMetadata(l, { fetchFn: i }))),
      !a)
    )
      try {
        a = await discoverOAuthProtectedResourceMetadata(t, { resourceMetadataUrl: h }, i);
      } catch {}
    ...
    let _ = await discoverOAuthServerInfo(t, { resourceMetadataUrl: h, fetchFn: i });
  ...
  if (N?.refresh_token)
    try {
      let _ = await refreshAuthorization(l, {
        metadata: p,
        clientInformation: f,
      ...
      if (!(_ instanceof OAuthError) || _ instanceof ServerError);
      else throw _;
    }
  let te = e.state ? await e.state() : void 0,
    { authorizationUrl: re, codeVerifier: oe } = await startAuthorization(l, {
      metadata: p,
      clientInformation: f,
      state: te,
...
function extractWWWAuthenticateParams(e) {
...
async function discoverAuthorizationServerMetadata(e, { fetchFn: t = fetch, protocolVersion: r = LATEST_PROTOCOL_VERSION } = {}) {
  ...
  for (let { url: i, type: u } of n) {
    ...
    if (u === "oauth") return OAuthMetadataSchema.parse(await a.json());
    else return OpenIdProviderDiscoveryMetadataSchema.parse(await a.json());
  }
  return;
}
async function discoverOAuthServerInfo(e, t) {
  ...
  let n = await discoverAuthorizationServerMetadata(o, { fetchFn: t?.fetchFn });
  return {
...
async function D(
  ...
  return OAuthTokensSchema.parse(await p.json());
}
