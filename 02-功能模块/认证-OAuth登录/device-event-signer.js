...
function h(r) {
  let t = [],
    ...
    s = (e) => {
      if (e === null || typeof e === "boolean" || typeof e === "string")
        t.push(jsonStringifyUntraced(e));
  ...
  while (i.length > 0) {
    ...
    if (e.next > 0) t.push(",");
    if (e.names) t.push(`${jsonStringifyUntraced(e.names[e.next])}:`);
...
var u = Buffer.from([0]);
function g(r, t) {
  let i = DEVICE_REGISTRY_KID_PREFIX + r,
    s = !1;
  return {
    noteSignFailure: () => {
      if (s) return !1;
...
async function deviceEventSignerOnceBoundHere(r, t) {
  let i = await resolveAccountIdentity(t).catch(() => {
    return;
  });
...
async function deviceEventSignerFor(r, t) {
  let i = await resolveAccountIdentity(t).catch(() => {
    return;
  });
...
async function deviceEventSignerIfBoundHere(r, t) {
  let i = await resolveAccountIdentity(t).catch(() => {
    return;
  });
  if (i?.status !== "resolved") return;
...
export { deviceEventSignerOnceBoundHere, deviceEventSignerFor, deviceEventSignerIfBoundHere };
