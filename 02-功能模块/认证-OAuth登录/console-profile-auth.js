...
import { clearTrustedDeviceTokenCache } from "../远程控制-Bridge/chunk-tyce0p0b.js";
...
import { clearOrgMemoryCredential } from "../记忆-CLAUDE.md/记忆-CLAUDE.md.vx19drc8.js";
import { emitAuthEvent } from "../../01-核心基础设施/遥测-OpenTelemetry/otel-events.js";
import { handleArtifactAccountChange, flushLiveReplicasBeforeLogout } from "../制品发布-Artifact/chunk-rr78st95.js";
import { withCredentialsLock, resetWIFCredentialState } from "./wif-credentials.js";
import { getAccountSettings, getGroveConfig, githubConnectionStatusStore } from "../隐私设置-Grove/chunk-a4mdm49v.js";
import { clearMeIdResolutionCache } from "../制品发布-Artifact/artifact-db.js";
...
function doesProfileLoginRefusalPermitFallback(e) {
...
function getProfileLoginRefusalCauseSummary(e) {
...
async function resolveConsoleProfileLoginTarget() {
  if (
  ...
  if (f !== null) {
    ...
    if (f.authentication.client_id !== CONSOLE_OAUTH_CLIENT_ID)
      throw new R(
...
async function q(e) {
  ...
  let c = {
    ...
    authentication: { type: "user_oauth", client_id: CONSOLE_OAUTH_CLIENT_ID },
    ...P,
  };
  await Iae(f, c);
  let g = i,
...
async function L({ revoke: e } = {}) {
  try {
    ...
    return (
      logForDebugging(`Logout: profile credential removal failed: ${l(o)}`, {
        level: "error",
      }),
      { removed: !1 }
    );
...
async function V(e) {
  ...
  let g = { refreshToken: c.refreshToken, clientId: CONSOLE_OAUTH_CLIENT_ID };
  if (e && g.refreshToken)
  ...
  try {
    ...
    return (
      logForDebugging(`Logout: could not remove profile credentials: ${l(p)}`, {
        level: "error",
      }),
      { removed: !1, ...g }
    );
  ...
  return (
    logForDebugging(`Logout: removed credentials for profile ${t}`),
    resetWIFCredentialState(),
    { removed: !0, ...g }
  );
}
function I(e) {
  let o = a.ANTHROPIC_PROFILE || getActiveProfileName(e);
  if (!D(o))
    throw new R(
...
async function k(e) {
  ...
  try {
    ...
    return (
      logForDebugging(`Profile config is not JSON, treating as foreign: ${l(f)}`),
...
function F(e) {
  return (
    ...
    e.authentication.client_id === CONSOLE_OAUTH_CLIENT_ID
  );
}
async function S(e) {
  let o;
...
async function isRefreshTokenStoredInProfile(e) {
  let o = getAnthropicConfigDir();
  if (o === null) return !1;
  try {
...
async function performLogout({
  ...
  if ((await E(), await flushLiveReplicasBeforeLogout(c), isBgSession())) {
...
async function clearAuthRelatedCaches(
  ...
  let c = B();
  if (
    (clearMeIdResolutionCache(c.host),
    handleArtifactAccountChange(t, i),
    ...
    await clearRemoteManagedSettings(e),
    await clearPolicyLimitsCache(),
    resetLimitGraceState(),
    resetNearLimitHintState(),
    !o)
  )
    (cancelAutoResume("account_switch"), endLowPriorityMode("account_switch"), clearLowPriorityBudgetSpent());
  resetObservedLimits();
}
...
export { CONSOLE_OAUTH_CLIENT_ID, CONSOLE_PROFILE_OAUTH_SCOPES, doesProfileLoginRefusalPermitFallback, getProfileLoginRefusalCauseSummary, resolveConsoleProfileLoginTarget, saveConsoleProfileLogin, isRefreshTokenStoredInProfile, performLogout, clearAuthRelatedCaches, fleetHostLogout };
