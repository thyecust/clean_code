...
function Jdt(e) {
  (ds().swarmPermissions.pending.set(e.requestId, e),
    n(
      `[SwarmPermissionPoller] Registered callback for request ${e.requestId}`,
    ));
}
function jqe(e) {
  (ds().swarmPermissions.pending.delete(e),
    n(`[SwarmPermissionPoller] Unregistered callback for request ${e}`));
}
function aWn(e) {
  return ds().swarmPermissions.pending.has(e);
}
function lWn() {
  ds().swarmPermissions.clear();
}
var d = !0;
function a(e, i) {
...
function Wqe(e) {
  let i = ds().swarmPermissions.pending,
    s = i.get(e.requestId);
  if (!s)
    return (
      n(
...
function cWn(e) {
  (ds().swarmPermissions.pendingSandbox.set(e.requestId, e),
    n(
      `[SwarmPermissionPoller] Registered sandbox callback for request ${e.requestId}`,
    ));
}
function uWn(e) {
  return ds().swarmPermissions.pendingSandbox.has(e);
}
function dWn(e) {
  let i = ds().swarmPermissions.pendingSandbox,
    s = i.get(e.requestId);
  if (!s)
    return (
      n(
...
export { Jdt, jqe, aWn, lWn, Wqe, cWn, uWn, dWn };
