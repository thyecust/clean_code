...
function De(e) {
  ...
  if (((t.lastResolvedAccountToken = e.token), r === void 0 || r === e.token))
    return;
  ((getMcpClientState().identityEpoch += 1), markIdentityChanged(), Je());
}
function Je() {
  ...
  queueMicrotask(() => {
    let t = getMcpClientState().identityTrippedHandler;
    if (!t) {
      getMcpClientState().identityTripUnowned = !0;
      return;
    }
    try {
      t();
    } catch (r) {
      logForDebugging(`MCP identity-change handler failed: ${l(r)}`, { level: "warn" });
    }
  });
}
function setIdentityChangeHandler(e) {
...
function consumeUnownedIdentityTrip() {
  let e = getMcpClientState(),
...
function resolveAccountTokenAndRecord() {
  let e = resolveAccountToken();
  return (De(e), e);
}
function getIdentityEpoch() {
  return (ue(), getMcpClientState().identityEpoch);
}
...
function rearmDiscoveryCacheBaseline() {
  if (!isDiscoveryCacheEnabled()) return;
  (bumpIdentityEpoch(), (getDiscoveryCacheStore().lastResolvedAccountToken = Qe));
}
function isCurrentIdentityEpoch(e) {
  return (ue(), e === getMcpClientState().identityEpoch);
}
...
function Ee() {
  return (ue(), getMcpClientState().identityChangedThisProcess);
}
var Ne = 4096,
  et = 1024,
  tt = new j(() => new ke());
...
function isRemoteTransportWithStaleIdentity(e, t) {
  return !isCurrentIdentityEpoch(t) && isRemoteTransport(e);
}
function buildIdentityChangedServer(e, t) {
  return {
    name: e,
    type: "failed",
    config: t,
...
function hasRemoteTransport(e) {
  return e.some((t) => isRemoteTransport(t.config));
}
function ft() {
  return resolveAccountTokenAndRecord().token;
}
var ye = 1,
  lt = 900,
  pt = 14400,
...
function isDiscoveryCacheEligible(e) {
  return Ue(e) === void 0;
}
function mt(e) {
...
function Ue(e) {
  ...
  if (isCliOwnedMcpConfig(e) || hasCliOwnedBearerProvider(e)) return "cli-owned";
  if (wt(e)) return "env-placeholder";
  if ((isFirstPartyDesignUrl(e.url) && !configHasAuthorizationHeader(e)) || isSessionIngressUrl(e.url)) return "ambient-credential";
  let r = e;
...
async function kt(e, t) {
  let r = resolveAccountTokenAndRecord();
...
async function getDiscoveryCacheLegToken(e, t) {
  if (!isDiscoveryCacheEnabled()) return;
  try {
...
async function checkDiscoveryCacheAdmission(e, t, r = Date.now()) {
  let i = Ue(t);
  if (i !== void 0 && !mt(i))
    return {
      kind: "miss",
...
function recordPresentedHeaders(e, t) {
...
function It(e, t, r = []) {
  ...
  for (let u of r) d(u);
  let p = getPresentedCredentialLog().snapshot();
...
async function Se(e) {
  ...
  if (!isCurrentIdentityEpoch(p.identityEpoch))
    return (
      logMCPDebug(
        r,
  ...
  let E = It(b, i, w);
  if (E !== void 0) {
    if (getPresentedCredentialLog().takeOverflowNotice())
      logMCPDebug(
        r,
...
async function awaitDiscoveryCacheFlush(e, t = 1500) {
  (K(), await Lt(e, t));
}
function K() {
  ...
  e.flushCleanup = registerCleanup(() => e.drain(DISCOVERY_CACHE_DRAIN_TIMEOUT_MS));
}
async function writeDiscoveryCacheEntry(e, t, r, i) {
  let o = i.now ?? Date.now(),
    d = x(e, t),
    p = q(d);
  if (!isCurrentIdentityEpoch(i.identityEpoch)) return "superseded";
  if (i.grantLeg === void 0) return "superseded";
  if (!isDiscoveryCacheEligible(t)) return "superseded";
  K();
  let u;
  try {
    u = await _(e, t);
...
async function mergeDiscoveryCacheTools(e, t, r, i) {
  if (!isCurrentIdentityEpoch(i.identityEpoch)) return;
  if (i.grantLeg === void 0) return;
  let o = x(e, t),
    d = q(o);
  if ((K(), !isDiscoveryCacheEligible(t))) return;
  let p;
  try {
    p = await _(e, t);
  } catch {
...
async function recordDiscoveryCacheStrike(e, t, r) {
  if (!isCurrentIdentityEpoch(r.identityEpoch)) return;
  ...
  await Y(p, e, async () => {
    let u = await Xe(d, e);
    if (!u) return;
    if (!isCurrentIdentityEpoch(r.identityEpoch)) {
      logMCPDebug(
        e,
...
function recordLazyDialFailureStrike(e, t) {
  (async () => {
    let r = getIdentityEpoch();
    await recordDiscoveryCacheStrike(e, t, { identityEpoch: r, grantLeg: await getDiscoveryCacheLegToken(e, t) });
  })().catch((r) =>
    logForDebugging(
...
async function deleteDiscoveryCacheEntry(e, t) {
  if (t.type !== "http" && t.type !== "sse") return;
  if (!isDiscoveryCacheUsable()) return;
  (K(), ae(x(e, t)));
  try {
...
async function acquireDiscoveryCacheRefreshLock(e, t) {
  let r;
  try {
    r = await bt(e, t);
  } catch {
