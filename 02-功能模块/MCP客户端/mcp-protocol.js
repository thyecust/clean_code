...
function isZ4Schema(e) {
  return !!e._zod;
}
function safeParse(e, t) {
  if (isZ4Schema(e)) return Ykt(e, t);
  return e.safeParse(t);
}
function getObjectShape(e) {
  if (!e) return;
  let t;
  if (isZ4Schema(e)) t = e._zod?.def?.shape;
  else t = e.shape;
  if (!t) return;
...
function assertToolsCallTaskCapability(e, t, r) {
  if (!e)
...
function assertClientRequestTaskCapability(e, t, r) {
  if (!e)
    throw Error(`${r} does not support task creation (required for ${t})`);
  switch (t) {
...
export { isZ4Schema, safeParse, getObjectShape, Protocol, mergeCapabilities, AjvJsonSchemaValidator, assertToolsCallTaskCapability, assertClientRequestTaskCapability };
