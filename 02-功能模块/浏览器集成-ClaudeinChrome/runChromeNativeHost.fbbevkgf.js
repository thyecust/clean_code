  ...
  async handleMessage(e) {
    ...
    try {
      ...
      (n("Invalid JSON from Chrome:", i.message),
        a(jsonStringify({ type: "error", error: "Invalid message format" })));
      return;
    }
    let s = E().safeParse(t);
    if (!s.success) {
    ...
    switch ((n(`Handling Chrome message type: ${r.type}`), r.type)) {
      case "ping":
        (n("Responding to ping"),
          a(jsonStringify({ type: "pong", timestamp: Date.now() })));
        break;
      case "get_status":
        a(jsonStringify({ type: "status_response", native_host_version: _ }));
        break;
      case "tool_response": {
        if (this.mcpClients.size > 0) {
          n(`Forwarding tool response to ${this.mcpClients.size} MCP clients`);
          let { type: i, ...c } = r,
            o = Buffer.from(jsonStringify(c), "utf-8"),
            l = Buffer.alloc(4);
      ...
      default:
        (n(`Unknown message type: ${r.type}`),
          a(jsonStringify({ type: "error", error: `Unknown message type: ${r.type}` })));
    }
  }
  handleMcpClient(e) {
    ...
    (this.mcpClients.set(t, s),
      ...
      a(jsonStringify({ type: "mcp_connected" })),
      e.on("data", (r) => {
        ...
        while (s.buffer.length >= 4) {
          ...
          try {
            let o = jsonParse(c.toString("utf-8"));
            (n(`Forwarding tool request from MCP client ${t}: ${o.method}`),
              a(
                jsonStringify({ type: "tool_request", method: o.method, params: o.params }),
              ));
          } catch (o) {
      ...
      e.on("close", () => {
        (n(
          ...
          a(jsonStringify({ type: "mcp_disconnected" })));
      }));
  }
}
class P {
