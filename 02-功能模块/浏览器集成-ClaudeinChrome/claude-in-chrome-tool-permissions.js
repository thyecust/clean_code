...
// Version: 2.1.263
import { AUTO_APPROVABLE_BROWSER_TOOL_NAMES } from "../图片-截图-ComputerUse/chunk-mk8kjx9c.js";
import { classifyChromeToolError } from "./chrome-tool-error-classifier.js";
import { executeBrowserToolCall } from "../图片-截图-ComputerUse/chunk-csvzwhzk.js";
        ...
        let n = t.result?.content,
          r =
            ...
            typeof n[0].text === "string"
              ...
              : void 0;
        if (!r) return;
        return jsonParse(r).availableTabs;
      })(),
      Z,
...
async function ne(e, t) {
  ...
  for (let o of e.content ?? [])
    if (o.type === "text") r.push({ type: "text", text: o.text });
      ...
      if (te(o.mimeType))
        try {
          let { block: s } = await buildImageBlock({
            data: String(o.data),
...
function oe(e) {
  ...
  for (let t of e.actions) {
    ...
    if (AUTO_APPROVABLE_BROWSER_TOOL_NAMES.has(t.name) && isAutoApprovableBrowserToolCall(t.name, n)) continue;
    if (
      t.name === "navigate" &&
      typeof n.url === "string" &&
...
function re(e) {
  ...
  return e.actions.some((t) => {
    ...
    return (
      ...
      (AUTO_APPROVABLE_BROWSER_TOOL_NAMES.has(t.name) && !isAutoApprovableBrowserToolCall(t.name, isRecord(t.input) ? t.input : {}))
    );
  });
}
function se(e, t, n) {
...
async function ae(e, t, n, r) {
  ...
  let i,
    ...
    d;
  try {
    d = await Promise.race([executeBrowserToolCall(o.context, o.socketClient, e, t ?? {}, r), m]);
  } finally {
...
function getClaudeInChromePermissionOverrides(e) {
  ...
  return {
    checkPermissions: async (o, s) => {
      let i = s.toolUseId,
        m = AUTO_APPROVABLE_BROWSER_TOOL_NAMES.has(e);
...
export { setChromeBinding, getClaudeInChromePermissionOverrides };
