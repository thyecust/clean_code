...
var e = String.raw`(?:No tab with id: \d+|Tab \d+ (?:no longer exists|is not in the same group))`,
  a = [
    ...
    {
      pattern: new RegExp(`^Failed to execute JavaScript: ${e}`),
  ...
  p = [
    ...
    {
      pattern: new RegExp(`^Failed to execute action: ${e}`),
      errorType: "tab_not_found",
    },
  ...
  s = [
    ...
    {
      pattern: new RegExp(`^Failed to extract page text: ${e}`),
      errorType: "tab_not_found",
    },
    {
  ...
  _ = [
    ...
    {
      pattern:
        /^Could not verify this site's safety category\. Blocking as a precaution/,
      errorType: "category_lookup_error",
    },
    { pattern: /^Claude in Chrome is turned off/, errorType: "domain_blocked" },
    {
      pattern: new RegExp(`^Tab \\d+ no longer exists|^${e}`),
      errorType: "tab_not_found",
    },
    {
      pattern:
...
function classifyChromeToolError(i, o) {
  let n = c.get(i);
  if (n) {
...
export { classifyChromeToolError };
