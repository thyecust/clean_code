...
import { Ab, Cb, vb } from "../../00-第三方库/https-proxy-agent/https-proxy-agent + undici.1t3vmhtr.js";
...
async function ne(e) {
  ...
  e.onDebug(
    `[runner:orchestrator] spawn-runner jti=${e.claims.jti} workOrderFile=${o}`,
  );
  let d = {
      ...process.env,
...
function le(e, t, r, o) {
  let d = je(r.apiUrl, e),
    n =
      e.hostRewrite && e.hostRewrite.from === e.host.toLowerCase()
        ? e.hostRewrite.toHost
        : e.host,
    s =
      e.hostRewrite && e.hostRewrite.from === e.host.toLowerCase()
        ? e.hostRewrite.toPort
        : e.port,
    w = !1,
    c = null,
    p = null,
  ...
  function O(m) {
    let i = c;
    if (!i || i.readyState !== WebSocket.OPEN || i.bufferedAmount <= Ie)
  ...
  async function x(m) {
    ...
    if (m.port !== void 0 && m.port !== 0 && m.port !== e.port) {
      S(
        i,
        `port ${m.port} does not match this connector's configured port ${e.port}`,
      );
      return;
    }
    let T = Fe(m.path);
    ...
    R.set(i, D);
    let ee = setTimeout((v) => v.abort(), Ae, D),
      I;
    try {
      let v = new Headers();
      ...
      let H =
        m.body !== void 0 && m.body !== ""
          ? Buffer.from(m.body, "base64")
          : void 0;
      I = await fetch(B, {
        method: K,
        headers: v,
        ...
        signal: D.signal,
        tls:
          _ !== void 0 || n !== e.host ? { ca: _, serverName: e.host } : void 0,
      });
    } catch (v) {
      (clearTimeout(ee),
        R.delete(i),
        g(`forward to ${e.host}: ${l(v)}`),
        S(i, l(v)));
      return;
    }
    clearTimeout(ee);
    ...
    try {
      ...
      (f({
        httpChunk: { requestId: i, data: "", isFinal: !0, sequenceNumber: q },
      }),
  ...
  function U() {
    ...
    ((c = m),
      ...
      (m.onmessage = (i) => {
        let T =
          typeof i.data === "string"
            ? i.data
            : Buffer.from(i.data).toString("utf8");
        A(T);
      }),
      (m.onerror = (i) => {
        g(
          "message" in i && typeof i.message === "string"
            ? i.message
            : "WebSocket error",
        );
      }),
      (m.onclose = (i) => {
        ...
        if (i.code !== oe)
          E(`closed code=${i.code} reason=${i.reason || "(none)"}`);
        L(i.code);
      }));
  }
  function G() {
      ...
      case "--hook-timeout":
        if (a) {
          ...
          if (!Number.isFinite(u) || u <= 0)
            throw Error(
              `--hook-timeout must be a positive number of seconds, got: ${a}`,
            );
      ...
      case "--expected-spawn-seconds":
        if (a) {
          ...
          if (Number.isNaN(u) || u < 10 || u > 3600 || String(u) !== a.trim())
            throw Error(
              `--expected-spawn-seconds must be an integer in [10, 3600], got: ${a}`,
            );
...
function _t(e) {
  ...
  if (typeof e === "string")
    return /^(0|[1-9]\d{0,14})$/.test(e) ? Number(e) : e;
  return String(e);
}
function pt(e, t) {
  ...
  return {
    ...
    client_platform:
      typeof e.client_platform === "string" ? e.client_platform : "",
  };
}
...
`));
    ...
    let w = b({
      ...
      last_poll_at:
        t.last_poll_at > 0 ? new Date(t.last_poll_at).toISOString() : null,
      last_hint_at:
        t.last_hint_at > 0 ? new Date(t.last_hint_at).toISOString() : null,
      last_hook_ok_at:
        t.last_hook_ok_at > 0
          ? new Date(t.last_hook_ok_at).toISOString()
          : null,
      last_warm_hook_ok_at:
        t.last_warm_hook_ok_at > 0
          ? new Date(t.last_warm_hook_ok_at).toISOString()
          : null,
  ...
  return (
    ...
    o.on("listening", () => {
      ...
      r(
        `[runner:health] orchestrator /healthz and /metrics listening on :${s}`,
      );
    }),
    o.listen(e),
    o
...
async function gt(e, t) {
  ...
  while (!t.aborted) {
    ...
    if (k.length > 0)
      d(
        `[runner:orchestrator] dispatching ${k.length} hint(s) (concurrency=${s.hookConcurrency})`,
      );
    if (E.length > 0)
      d(
        `[runner:orchestrator] dispatching ${E.length} warm hint(s) (min_idle=${s.minIdle})`,
      );
  ...
  async function y(_, k, R) {
    ...
    let h = ml(
      ...
      f =
        g.exitCode === 1 || g.timedOut || (g.exitCode === null && !g.execError),
      S = !E
        ? void 0
        : g.timedOut
...
Run 'claude self-hosted-runner orchestrator --help' for usage.`),
  ...
  let N = mot({ baseUrl: t.apiUrl, poolSecret: c, onDebug: n }),
    ...
    E = () => {
      ...
      ((g = !0),
        s("Received shutdown signal, stopping poll loop..."),
        k.abort());
    };
