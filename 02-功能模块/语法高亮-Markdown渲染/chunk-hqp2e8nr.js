    ...
    let c = String(Re(this.hunk)).length,
      ...
      b = this.hunk.lines.map((m) => {
        let y = We(m.slice(0, 1)),
          { code: L, truncatedChars: M } = Rye(m, 1),
          S;
        switch (y) {
          case "+":
            S = f++;
...
`);
    ...
    for (let g = 0; g < i.length; g++) {
      let { code: b, truncatedChars: k } = Rye(i[g]),
        C = Y(u, b, r);
...
function dUn() {
  return WWe() === null ? I : null;
}
function VZt() {
  return WWe() === null ? j : null;
}
function KZt(e) {
  return WWe() === null ? ne(e) : null;
}
export { Mit, Rye, lle, WWe, dUn, VZt, KZt };
