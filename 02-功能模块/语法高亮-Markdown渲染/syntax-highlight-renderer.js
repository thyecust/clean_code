...
function truncateCodeLine(e, n = 0) {
  if (e.length - n <= MAX_CODE_LINE_CHARS) return { code: e.slice(n), truncatedChars: 0 };
  let t = e.slice(n, n + MAX_CODE_LINE_CHARS + 1),
    s = truncateToCodeUnits(t, MAX_CODE_LINE_CHARS);
  return { code: s, truncatedChars: e.length - n - s.length };
}
function formatTruncationNotice(e) {
  return ` \u2026 [+${e} chars]`;
}
function le() {
...
function ce(e, n, t) {
  let s = (y) =>
      y < 48 ? 0 : y < 115 ? 1 : y < 155 ? 2 : y < 195 ? 3 : y < 235 ? 4 : 5,
    r = s(e),
    i = s(n),
    o = s(t),
    d = 16 + 36 * r + 6 * i + o,
...
function W(e, n, t) {
  ...
  return `\x1B[${s};5;${ce(e.r, e.g, e.b)}m`;
}
function ae(e, n, t, s) {
  let r = s ? E + A : E;
  for (let [i, o] of e) {
...
`;
  ...
  let r;
  try {
    r = le().highlight(s, { language: e.lang, ignoreIllegals: !0 });
  } catch {
    return [[D(t), s]];
  }
  if (!be(r._emitter)) {
...
function Z(e, n, t, s) {
  ...
  for (let o = 0; o < e.lines.length; o++) {
    let d =
        o === 0 ? ` ${String(e.lineNumber).padStart(t)} ` : " ".repeat(t + 2),
      u = i && !s ? `${A}${d}${U}` : d;
...
function ee(e, n, t, s) {
  return e.lines.map((r) => ae(r, s, t, n));
}
function Re(e) {
...
class I {
  ...
  render(e, n, t, s) {
    ...
    let c = String(Re(this.hunk)).length,
      ...
      b = this.hunk.lines.map((m) => {
        let y = We(m.slice(0, 1)),
          { code: L, truncatedChars: M } = truncateCodeLine(m, 1),
          S;
        switch (y) {
          case "+":
            S = f++;
    ...
    for (let m = 0; m < b.length; m++) {
      ...
      if ((V(T), Be(T, o, k[m]), S > 0)) T.lines[0].push([D(o), formatTruncationNotice(S)]);
...
`);
    ...
    for (let g = 0; g < i.length; g++) {
      let { code: b, truncatedChars: k } = truncateCodeLine(i[g]),
        C = Y(u, b, r);
      if (k > 0) C.push([D(r), formatTruncationNotice(k)]);
...
function getSyntaxHighlightOverrideSource() {
  if (po(a.CLAUDE_CODE_SYNTAX_HIGHLIGHT)) return "env";
  return null;
}
function getDiffHunkRenderer() {
  return getSyntaxHighlightOverrideSource() === null ? I : null;
}
function getCodeBlockRenderer() {
  return getSyntaxHighlightOverrideSource() === null ? j : null;
}
function getSyntaxThemeDescriptor(e) {
  return getSyntaxHighlightOverrideSource() === null ? ne(e) : null;
}
export { MAX_CODE_LINE_CHARS, truncateCodeLine, formatTruncationNotice, getSyntaxHighlightOverrideSource, getDiffHunkRenderer, getCodeBlockRenderer, getSyntaxThemeDescriptor };
