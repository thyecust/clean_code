...
import { getYieldArtifactRepliesSchema, getArtifactRepliesYieldedSchema, getUnyieldArtifactRepliesSchema, hasOutstandingYieldWait, handleYieldAnswer, setReplyYieldSender, handleYieldRequest, handleUnyieldRequest } from "../制品发布-Artifact/artifact-reply-yield.js";
import {
  notifyWhenIdleFrameSchema,
  peerIdleNoticeFrameSchema,
  recordIdleSubscription,
  flushIdleSubscribers,
  hasOutstandingIdleSubscription,
  admitIdleNotice,
  setIdleNoticeSender,
  setRegisteredInboxOfPidReader,
...
function getUdsStartFailureDetail(e) {
...
function getUdsStartFailureCause() {
  return c().lastStartFailureCause;
}
function getUdsStartDegradedCause() {
...
function getUdsInboxUnavailableReason() {
  return getUdsStartFailureDetail(c());
}
function me(e) {
...
function setOnRename(e) {
  c().onRename = e;
}
function setOnEnableRemoteControl(e) {
...
function Qe(e) {
  ...
  let t = [];
  for (let i of e) {
    if (t.length >= MAX_TRACKED_MESSAGE_IDS) break;
    if (isUuidString(i)) t.push(i);
  }
  return t;
}
function setOnPeerMessageStatus(e) {
  c().onPeerMessageStatus = e;
}
function setOnEnqueue(e) {
  c().onEnqueue = e;
}
function Te(e) {
  if (e.session_id !== void 0 && e.session_id !== K())
    return (
      logForDebugging(
        `[uds-messaging] Dropping ${formatRedactedPreview(e.type)} message: session_id mismatch (got "${formatRedactedPreview(String(e.session_id))}", expected "${K()}")`,
        { level: "warn" },
      ),
      !1
    );
...
async function Je(e, t, i, r, d) {
  ...
  if (e.file_attachments !== void 0 && isSendFileEnabled())
    try {
      ...
      logForDebugging(
        `[uds-messaging] Failed to materialize file_attachments: ${formatRedactedErrorDetail(String(m))}`,
        { level: "warn" },
      );
    }
  ...
  (enqueueCommand(k),
    logForDebugging(
      `[uds-messaging] Routed user message to queue (priority=${p}): ${formatRedactedPreview(E, 80)}`,
    ),
    c().onEnqueue?.(),
    Me(k));
}
function Me(e) {
  ...
  if (
    ...
    i !== void 0 &&
    vettedPeerReplyTarget(t.from, i, t.verifiedPeerPid) !== void 0
  )
    noteVettedCorrespondent(t.from, t.verifiedPeerPid, t.verifiedPeerProcStart);
}
function vettedPeerReplyTarget(e, t, i) {
...
async function ke(e, t, i, r, d) {
  if (!me(e)) {
    logForDebugging("[uds-messaging] Ignoring message without valid type field", {
      level: "warn",
    });
    return;
  }
  if (e.type === "user") await Je(e, t, i, r, d);
    ...
    if (e.action === "rename" && typeof e.name === "string")
      ...
      let s =
        ...
        o = w?.destination;
      if (e.status === "dropped") {
        let p = parseDropReason(e.drop_reason),
      ...
      let s = notifyWhenIdleFrameSchema().safeParse(e);
      if (!s.success) {
        (logForDebugging("[uds-messaging] notify_when_idle dropped: malformed frame"),
          logFeatureSad("cross_session_notify_idle", "malformed_frame"));
        return;
      }
      let w = c().activeSocketPath,
        o = s.data.from,
        p = w !== void 0 ? vettedPeerReplyTarget(o, w, t) : void 0;
      if (w === void 0)
        (logForDebugging(
        ...
        (logForDebugging(
          `[uds-messaging] notify_when_idle dropped: reply address unshaped or outside our socket namespace (${formatRedactedPreview(o)})`,
        ),
        ...
        if (
          (logForDebugging(`[uds-messaging] notify_when_idle from ${formatRedactedPreview(o)}: ${u}`),
          u === "full")
        )
          sendUnavailableNotice(p, s.data.msg_id, t, d === "peer", i);
      }
    } else if (e.action === "peer_idle_notice") {
      let s = peerIdleNoticeFrameSchema().safeParse(e);
      if (!s.success) {
        (logForDebugging("[uds-messaging] peer_idle_notice dropped: malformed frame"),
          logFeatureSad("cross_session_notify_idle", "malformed_notice"));
        return;
      }
      if (!hasOutstandingIdleSubscription(s.data.orig_msg_id)) {
        logForDebugging(
      ...
      if (
        ...
        logForDebugging(
          `[uds-messaging] peer_idle_notice not admitted: subscription for orig_msg_id=${formatRedactedPreview(s.data.orig_msg_id)} was already consumed`,
        );
      ...
      let w = c().activeSocketPath,
        o = w !== void 0 ? vettedPeerReplyTarget(s.data.from, w, t) : void 0;
      if (w === void 0 || o === void 0) {
        (logForDebugging(
          `[uds-messaging] yield_artifact_replies dropped: ${w === void 0 ? "own inbox not bound" : "reply address unshaped or outside our socket namespace"} (${formatRedactedPreview(s.data.from)})`,
        ),
      ...
      if (
        ...
        (logForDebugging(
          `[uds-messaging] yield_artifact_replies refused: requester is not a verified live session of this conversation (${formatRedactedPreview(s.data.from)})`,
        ),
      ...
      logForDebugging(`[uds-messaging] Unhandled control action: ${formatRedactedPreview(String(e.action))}`);
  } else logForDebugging(`[uds-messaging] Received unhandled message type: ${formatRedactedPreview(e.type)}`);
}
function Ze(e, t, i, r, d) {
  if (
    ...
    ke(e, t, i, r, d).catch((s) => {
      logForDebugging(`[uds-messaging] Failed to process message: ${s}`, { level: "warn" });
    });
    return;
  }
  c().processingChain = c()
  ...
  let t = c().firstLineDeadlineMs,
    i = setTimeout(() => {
      ...
      try {
        ...
        logForDebugging(`[uds-messaging] Failed to close a silent connection: ${S}`, {
          level: "warn",
        });
      }
    }, t);
  ...
  let u = "",
    ...
    b = (S) => {
      ...
      if (c().authRequired && _ === void 0) {
        m(me(S) ? `a '${formatRedactedPreview(S.type)}' line` : "a line");
        return;
      }
      if (!w) {
        if (
          ((d = getSocketPeerPid(e)),
          (s =
            d === void 0
              ? void 0
              : p !== void 0
    ...
    D = (S) => {
      try {
        ...
        logForDebugging(`[uds-messaging] Failed to handle line: ${R}`, { level: "warn" });
      }
    };
  (e.on("data", (S) => {
    if (((u += S), u.length > MAX_MESSAGE_CHARS)) {
      (logForDebugging(`[uds-messaging] Line exceeded ${MAX_MESSAGE_CHARS} chars; dropping connection`, {
        level: "warn",
      }),
        e.destroy(),
        (u = ""));
...
`)) !== -1
    ...
    e.on("error", (S) => {
      logForDebugging(`[uds-messaging] Connection error: ${S.message}`, { level: "warn" });
    }));
}
...
async function H(e, t, i, { settleHeld: r = !0 } = {}) {
  ...
  if ((c().connectedClients.clear(), e.close(), r)) noteParkedAtShutdown();
  let d = r ? settleHeldPeerMessagesOnShutdown() : void 0;
  await nn();
  let s = r ? withDeadline(flushIdleSubscribers("exited"), en) : void 0;
...
function Y() {
  ((c().activeSocketPath = void 0),
    ...
    setOwnUdsHopToken(void 0),
    setPeerReceiptSender(null),
    setCorrespondentRecorder(null),
    setIdleNoticeSender(null),
    setReplyYieldSender(null),
    setRegisteredInboxOfPidReader(null),
...
function startCrossSessionInbox(e, t) {
  return startUdsMessaging(e ?? getDefaultUdsSocketPath(), t, { isExplicit: e !== void 0 });
}
function q() {
...
async function validateExplicitMessagingSocketPath(e) {
  if (!isAbsolute(e))
    throw new ud(
      e === ""
...
async function startUdsMessaging(e, t, i = {}) {
  c().startInFlight = !0;
  try {
...
async function gn(e, t, i = {}) {
  ...
  if (i.isExplicit) {
    if (((e = await validateExplicitMessagingSocketPath(e)), (await ge(e)) === "live"))
      throw new ud(
  ...
  try {
    ...
    (Y(),
      logForDebugging(`[uds-messaging] Failed to create server: ${String(o)}`, {
        level: "error",
      }),
      q());
    return;
  ...
  (d.on("error", (o) => {
    logForDebugging(`[uds-messaging] Server error: ${o.message}`, { level: "error" });
  }),
  ...
  try {
    ...
    d.unref();
    let o = registerCleanup(async () => {
      (logForDebugging("[uds-messaging] Shutting down"), await H(d, e, t));
    });
    ...
    ((process.env.CLAUDE_CODE_MESSAGING_SOCKET = e),
      ...
      setOwnUdsHopToken(buildUdsAddress(e)));
    let p = buildUdsAddress(e);
    return (
      setCorrespondentRecorder(Me),
      (getSessionNamingState().senderMode = currentSenderClass),
      setIdleNoticeSender((u, _, k, m, b) =>
        sendControlToUdsSocket(
          u,
      ...
      setRegisteredInboxOfPidReader((u) => registeredInboxesOfPids(u)),
      setPeerReceiptSender((u, _, k) => {
        let m = u.origin?.kind === "peer" ? u.origin : void 0,
          b = m?.from;
        if (typeof b !== "string") return;
        let D = vettedPeerReplyTarget(b, e, m?.verifiedPeerPid);
        if (D === void 0) {
          logForDebugging(
            `[uds-messaging] hold-receipt skipped: reply address unshaped or outside our socket namespace (${formatRedactedPreview(b)})`,
          );
          return;
        }
        return sendControlToUdsSocket(
          ...
          logForDebugging(
            `[uds-messaging] hold-receipt send failed to ${formatRedactedPreview(b)}: ${formatRedactedErrorDetail(String(S))}`,
          ),
        );
      }),
      logForDebugging(`[uds-messaging] Listening: ${e}`, { level: "info" }),
      logForDebugging(
    ...
    else logForDebugging(`[uds-messaging] Failed to start: ${o}`, { level: "error" });
    (Y(), q());
    return;
  }
}
...
function hn() {
  let e = currentSenderClass();
  return e === void 0 ? {} : { from_mode: e };
}
function currentSenderClass() {
...
function yn() {
  if (!process.listeners("exit").includes(unlinkActiveKeyFileSync)) process.on("exit", unlinkActiveKeyFileSync);
}
function unlinkActiveKeyFileSync() {
  try {
...
export {
  ...
  udsMessagingStateStore,
  getUdsStartFailureCause,
  getUdsStartDegradedCause,
  getUdsInboxUnavailableReason,
  setOnRename,
  setOnEnableRemoteControl,
  setOnPeerMessageStatus,
  setOnEnqueue,
  vettedPeerReplyTarget,
  getUdsMessagingSocketPath,
  getDefaultUdsSocketPath,
  getPerUidFallbackUdsSocketPath,
  startCrossSessionInbox,
  validateExplicitMessagingSocketPath,
  startUdsMessaging,
  currentSenderClass,
  unlinkActiveKeyFileSync,
};
