...
import { isUuidString } from "../远程控制-Bridge/bridge-state-containers.js";
...
import { isSaneEpochMs } from "../守护服务-Daemon/process-record.js";
import { s, se, c, k } from "../../00-第三方库/zod/zod.5ef0bk11.js";
import { countMatching } from "../../01-核心基础设施/核心工具-数组与集合/chunk-d16fhdtx.js";
var R = 43200000,
  MAX_IDLE_SUBSCRIPTIONS = 32,
  V = 3,
  L = 750,
  z = 30000,
  X = 750,
  D = 100,
  notifyWhenIdleFrameSchema = createLazyValue(() =>
    c({
  ...
  peerIdleNoticeFrameSchema = createLazyValue(() =>
    c({
...
var d = new F(),
  ...
  Y = 4,
  Q = 4;
function recordIdleSubscription(e, i, t, r, o, a, _, v) {
  if (!isUuidString(t))
  ...
  let p = u.subscribers,
    ...
    w = () => p.length >= MAX_IDLE_SUBSCRIPTIONS;
  if (I !== -1) p.splice(I, 1);
  else if (r !== void 0) {
  ...
  if (
    ...
    logForDebugging(
      `[peer-idle] subscription ${I === -1 ? "recorded" : "refreshed"} for ${formatRedactedPreview(e)} (${r === void 0 ? "no peer pid" : `pid ${r}`}${a ? ", authenticated" : ""}; ${p.length} live)`,
    ),
    !A)
  )
    U(e, r !== void 0);
...
function notePeerIdleStatus(e, i) {
  let t = d;
...
function Z() {
  ...
  d.parkedHoldBack = !1;
  let e = flushIdleSubscribers("idle").finally(() => {
    d.inflight.delete(e);
  });
  d.inflight.add(e);
}
async function flushIdleSubscribers(e) {
  let i = d;
  if (e === "exited") {
...
async function J(e, i, t, r, o) {
  ...
  if (t.some((l) => l.verifiedPeerPid !== void 0))
    try {
      ...
      logForDebugging(
        `[peer-idle] registry unreadable at fire time; notices carry no detail (${formatRedactedErrorDetail(String(l))})`,
      );
    }
  if (getInboundPolicy() === "refuse") {
    logFeatureSad("cross_session_notify_idle", "revoked_before_fire");
    ...
    t.map((l) => {
      ...
      return i(
        ...
        (A) => {
          ...
          logForDebugging(
            `[peer-idle] notice to ${formatRedactedPreview(l.replyAddress)} failed: ${formatRedactedErrorDetail(String(A))}`,
          );
        },
      );
    }),
...
function ee() {
  ...
  try {
    ...
    logForDebugging(`[peer-idle] last-turn text unavailable: ${formatRedactedErrorDetail(String(t))}`);
  }
...
function registerIdleSubscription(e, i, t) {
  let r = d.outstanding,
  ...
  if (r.length - v.length >= MAX_IDLE_SUBSCRIPTIONS) return { ok: !1, reason: "cap" };
  let b = setTimeout(ne, R, e);
  return (
    b.unref?.(),
    r.push({
      msgId: e,
      label: safeIdleLabel(i),
      target: t,
      targetKey: o,
      conversationId: K(),
      requestedAt: Date.now(),
...
function hasOutstandingIdleSubscription(e) {
  return typeof e === "string" && d.outstanding.some((i) => i.msgId === e);
}
function forgetIdleSubscription(e) {
  let i = d,
...
function admitIdleNotice(e, i, t, r, o, a) {
...
function C(e) {
  let i = d,
    t = i.onNotice;
  if (t === null) {
    if (i.pendingNotices.length < MAX_IDLE_SUBSCRIPTIONS)
      i.pendingNotices.push({ admitted: e, conversationId: K() });
    else i.pendingNoticeOverflow++;
    logForDebugging(
      `[peer-idle] notice for "${formatRedactedPreview(e.label)}" (${e.kind}) buffered until a host handler mounts`,
    );
    return;
  }
  let r = te(e);
...
function setOnPeerIdleNotice(e) {
  let i = d;
...
function setIdleLastTurnTextProvider(e) {
  d.getLastTurnText = e;
}
function setIdleNoticeSender(e) {
  if (e !== null) d.exited = !1;
  d.sendNotice = e;
}
function setOnIdleSubscribed(e, i = null) {
...
function U(e, i, t = !1) {
  ...
  if (r.onSubscribed === null) {
    if (r.pendingAnnounces.length < MAX_IDLE_SUBSCRIPTIONS) r.pendingAnnounces.push([e, i]);
...
function setRegisteredInboxOfPidReader(e) {
  d.registeredInboxOfPid = e;
}
function idleSubscribedSystemLine(e, i) {
...
function idleNoticeSystemLine(e) {
  switch (e.kind) {
    case "idle": {
...
function noteParkedAtShutdown() {
  d.parkedHoldBack = getHeldPeerMessageCount() > 0;
}
function setHostStatusSubscription(e) {
...
function sendUnavailableNotice(e, i, t, r, o, a = !1) {
  let _ = d.sendNotice;
  ...
  _(e, { orig_msg_id: i, state: "unavailable" }, t, H(t, r), o).then(
    ...
    (v) => {
      ...
      logForDebugging(`[peer-idle] unavailable notice to ${formatRedactedPreview(e)} failed: ${formatRedactedErrorDetail(String(v))}`);
    },
  );
}
function de(e, i, t, r, o) {
  (d.unavailableRetries.delete(i), sendUnavailableNotice(e, i, t, r, o, !0));
}
function H(e, i) {
  return e !== void 0 || i;
}
function enqueueIdleNoticesForModel(e) {
...
export {
  ...
  safeIdleLabel,
  registerIdleSubscription,
  hasOutstandingIdleSubscription,
  forgetIdleSubscription,
  admitIdleNotice,
  setOnPeerIdleNotice,
  setIdleLastTurnTextProvider,
  setIdleNoticeSender,
  setOnIdleSubscribed,
  setRegisteredInboxOfPidReader,
  idleSubscribedSystemLine,
  idleNoticeSystemLine,
  noteParkedAtShutdown,
  setHostStatusSubscription,
  sendUnavailableNotice,
