...
async function listAllPeers(e, n, i) {
  let s = !1,
    t = isCrossSessionMessagingEnabled();
  if (t) await refreshPeerIdentityOwner({ refresh: !0, credentials: i });
  let l = t
      ? loadBridgePeerSessionRows(e, i)
  ...
  let a = excludeLocallyKnownBridgeRows(c.rows, g, r.sessions);
  if (!isCloudListUnavailable(r.unavailable)) recordBridgeSessionRows(e, c, excludeCloudKnownBridgeRows(c.rows, r.sessions));
  else forgetBridgeSessionRows(e, c);
  for (let d of a)
  ...
  return {
    ...
    didFocused: s,
    bridgeWalkFailed: c.failed || (c.identityKey !== null && !isBridgeRowsIdentityCurrent(c)),
    cloudListFailed: isCloudListUnavailable(r.unavailable),
    localListFailed: u,
    messagingDisabled: !t,
...
async function buildSubagentExtras(e, n) {
  let i = e.getAppState(),
    ...
    c = [
      ...
      ...i.map((a) => {
        ...
        return P(d ? formatAgentDisplayName(d) : (normalizePlainName(a.title) ?? "(untitled)"), [
          a.remoteControl ? REMOTE_CONTROL_LABEL : "cloud session",
          a.offline ? "offline" : C(a.workerStatus, a.workerStatus),
          a.lastActive === void 0
            ? void 0
            : `active ${y(g - a.lastActive)} ago`,
          a.unreachableFromHere ? k : void 0,
          a.acceptsPeerMessages === !1 ? CANNOT_RECEIVE_MESSAGES_LABEL : void 0,
        ]);
      }),
      ...s.map(({ session: a }) => {
        let d = n.get(`bridge-session\x00${a.id}`);
        return P(d ? formatAgentDisplayName(d) : getSessionTitleOrDefault(a.title), [
          getSessionTransportLabel(a),
          M(a, a.status),
          a.acceptsPeerMessages === !1 ? CANNOT_RECEIVE_MESSAGES_LABEL : void 0,
        ]);
      }),
  ...
  return R(
    ...
    e.map((s) => {
      switch (s.transport) {
        ...
        case "cloud": {
          let t = s.session;
          return `  [${t.offline ? "offline" : C(t.workerStatus, t.workerStatus ?? "unknown")}]  \xB7  ${b(t.title) ?? "(untitled)"}  \xB7  ${t.remoteControl ? REMOTE_CONTROL_LABEL : "cloud"}${t.unreachableFromHere ? `  \xB7  ${k}` : ""}${t.acceptsPeerMessages === !1 ? `  \xB7  ${CANNOT_RECEIVE_MESSAGES_LABEL}` : ""}`;
        }
        case "bridge": {
          let t = s.session;
          return `  [${M(t, b(t.status) ?? "unknown")}]  \xB7  ${b(t.title) ?? "(untitled)"}  \xB7  ${getSessionTransportLabel(t)}${t.acceptsPeerMessages === !1 ? `  \xB7  ${CANNOT_RECEIVE_MESSAGES_LABEL}` : ""}`;
        }
        case "did":
          return "";
      }
...
export { listAllPeers, buildSubagentExtras, formatForModel, formatForUser };
