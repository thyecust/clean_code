...
class se {
  ...
  async openConnection() {
    ...
    try {
      ...
      (n(`[deviceBridge] open connection failed: ${ge(e).message}`),
        this.endRotation(),
        this.scheduleReconnect());
    }
  }
  ...
  emit(e) {
    try {
      ...
      n(`[deviceBridge] liveness event listener threw: ${ge(t).message}`);
    }
  }
  notifyTransportListener(e) {
    try {
      ...
      n(`[deviceBridge] transport event listener threw: ${ge(t).message}`);
    }
  }
  clearReconnectTimer() {
...
function ye() {
  ...
  return {
    ...
    setGroup(p, v, C = "tools_changed") {
      ...
      let S = ae(e, t),
        _ = r;
      if (((r = S), S.dropped.length > 0 && b(S.dropped) !== b(_.dropped)))
        n(
          `[deviceBridge] duplicate device tool names not announced: ${b(S.dropped)}`,
        );
...
function ae(e, t) {
  ...
  return { byName: o, definitions: a, dropped: r, hash: Fxt(b(a)) };
}
var He = 0,
  ne = 1,
  ze = -32603,
...
class ce {
  ...
  async send(e) {
    ...
    try {
      ...
      (n(`[deviceBridge] outbound send failed: ${ge(o).message}`),
        this.emit({
          kind: "outbound_dropped",
          messageType: de(e) === void 0 ? "other" : "request",
        }));
  ...
  sendRaw(e) {
    ...
    try {
      ...
      return (n(`[deviceBridge] raw send failed: ${ge(o).message}`), !1);
    }
    return !0;
  }
  isAuthenticated() {
  ...
  async openConnection() {
    ...
    try {
      ((o = this.options.getUrl()),
        (r = (this.options.isAllowedUrl ?? h5t)(o)));
    } catch (l) {
  ...
  async resolveAccessToken() {
    ...
    try {
      ...
      n(`[deviceBridge] access token resolution failed: ${ge(t).message}`);
      return;
    } finally {
  ...
  disconnect() {
    ...
    if (
      ...
      try {
        ...
        n(`[deviceBridge] socket close failed: ${ge(t).message}`);
      }
  }
  emit(e) {
    try {
  ...
  handleMessage = (e) => {
    ...
    if (t.kind === "oversize") {
      (n(`[deviceBridge] dropping ${t.size}-unit inbound frame (cap ${te})`),
        this.emit({
          kind: "inbound_invalid",
...
function Ee(e) {
  let t = () => b([e.definitionsOf(ue), e.definitionsOf(K)]);
  return {
    ...
    : [
        {
          ...
          protocol_versions: t.protocol_versions ?? [pE],
...
function c6e(e) {
  let t = e.transport ?? (isSessionChannelDisabled() ? "bridge" : "auto"),
    ...
    I = () => {
      ...
      B = (x ?? Promise.resolve())
        ...
        .catch((R) => {
          n(`[deviceBridge] re-dial failed: ${ge(R).message}`);
        })
        .finally(() => {
          if (((B = void 0), w && z())) I();
        });
    ...
    D = async () => {
      ...
      if (!ee || N.status !== "resolved")
        return (
          ...
          n(`[deviceBridge] skipped: ${ee ? `account ${N.status}` : "no org"}`),
          !1
        );
  ...
  r = (e.registerExitCleanup ?? Et)(() => H("exit"));
  let Q = D().catch((d) => {
      if (
