...
class b {
  ...
  async callTool(e, n, t) {
    ...
    if (!this.ws || this.ws.readyState !== g.OPEN)
      throw new Pv(`[${i}] Bridge not connected`);
    ...
    if (
      ...
      throw new bGe(`[${i}] No Chrome extension connected after discovery`);
    ...
    if (a) {
      ...
      if (!this.ws || this.ws.readyState !== g.OPEN)
        throw new Pv(`[${i}] Bridge disconnected before tool call was sent`);
      if (v !== void 0 && this.selectedDeviceId === void 0)
        throw new qSe("Chrome extension disconnected mid-call");
    }
    let d = crypto.randomUUID(),
      l = Date.now(),
      u = this.context.getToolCallTimeoutMs?.(e) ?? Din;
  ...
  sendExternalMessage(e, n, t) {
    ...
    if (!this.ws || this.ws.readyState !== g.OPEN)
      return Promise.reject(new Pv(`[${i}] Bridge not connected`));
    ...
    let s = crypto.randomUUID(),
      r = t?.timeoutMs,
      c = Math.min(
        typeof r === "number" && Number.isFinite(r) && r > 0 ? r : Min,
        pWn,
      ),
      a = {
  ...
  async discoverAndSelectExtension() {
    ...
    if (t.length === 0) {
      if (
        (e.info(
          `[${n}] No extensions connected, waiting up to ${Qdt}ms for peer_connected`,
        ),
        await this.waitForPeerConnected(Qdt))
      )
    ...
    (this.ws.on("open", () => {
      ...
      this.ws.on("close", (c) => {
        ...
        (e.info(
          ...
          this.rejectPendingCalls(new Pv("Bridge connection closed mid-call")),
          this.scheduleReconnect());
      }),
      this.ws.on("error", (c) => {
        ...
        (e.error(`[${n}] Bridge WebSocket error after ${a}ms: ${c.message}`),
          ...
          this.rejectPendingCalls(
            new Pv(`Bridge connection error: ${c.message}`),
          ));
      }));
  }
  handleMessage(e) {
      ...
      case "peer_disconnected": {
        ...
        if (
          ...
          (n.info(`[${t}] Selected extension disconnected, clearing selection`),
            ...
            (this.discoveryComplete = !1),
            this.rejectPendingCalls(
              new qSe("Chrome extension disconnected mid-call"),
            ));
  ...
  createTimeoutTimer(e, n) {
    ...
    return setTimeout(() => {
      ...
      if (
        ...
        s.reject(new wGe(`[${o}] Tool call timed out: ${s.toolName}`));
      else
        s.reject(
          new Pv(
    ...
    this.keepAliveInterval = setInterval(() => {
      ...
      if (this.lastPongReceived > 0 && Date.now() - this.lastPongReceived > x)
        (e.warn(`[${n}] No pong received in ${x}ms, closing dead connection`),
          this.rejectPendingCalls(
            new Pv("Bridge keepalive timeout \u2014 connection dead"),
          ),
          this.closeSocket(),
          this.scheduleReconnect());
    }, A);
  ...
  cleanup() {
    ...
    (this.rejectPendingCalls(new Pv("Bridge client disconnected")),
...
class I {
  ...
  async callTabsContext(e, n) {
    ...
    if (i.length === 0) throw new Pv(`[${o}] No connected sockets available`);
    if (i.length === 1) {
    ...
    throw new Pv(`[${o}] All sockets failed for tabs_context_mcp`);
  }
  updateTabRoutes(e, n) {
...
function njn(e) {
  return e.bridgeConfig ? w(e) : e.getSocketPaths ? k(e) : lNt(e);
}
function aNt(e, n) {
  ...
  return (
    s.setRequestHandler(ListToolsRequestSchema, () => {
      ...
      return {
        tools: [
          ...(e.bridgeConfig ? D1t : D1t.filter((a) => !fWn.has(a.name))).map(
            (a) =>
    ...
    s.setRequestHandler(CallToolRequestSchema, async (r) => {
      ...
      let c = e.getRendererSessionScope?.(r.params._meta),
        a = c
          ...
          : void 0,
        d = await _ut(e, i, r.params.name, r.params.arguments || {}, a);
      if (c && !d.isError) {
        let l = d._meta?.[T],
          u =
            typeof l === "number"
