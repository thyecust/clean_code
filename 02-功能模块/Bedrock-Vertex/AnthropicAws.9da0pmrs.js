  ...
  z = (e, t) => {
    let r = e
      ? e
          ...
          .filter(Boolean)
      : [];
    for (let n of t.split(",").map((o) => o.trim()))
    ...
    for (let n of e) {
      ...
      for (let [i, s] of G(n)) {
        ...
        if (V.has(a)) {
          ...
          else (t.set(i, z(t.get(i), s)), r.delete(a));
          continue;
        }
        if (s === m || !o.has(a)) {
...
var q = { error: d, warn: d, info: d, debug: d },
  T = new WeakMap();
function M(e, t) {
  let r = T.get(e);
...
var Y = (e, t) =>
    import("../../01-核心基础设施/共享小工具-未细化/fromIni.7gtjb5bg.js")
      .then(({ fromNodeProviderChain: r }) =>
        r({
          ...
          clientConfig: {
            ...(t != null ? { logger: t } : {}),
            requestHandler: new k.FetchHttpHandler({
              requestInit: (n) => ({ ...n }),
            }),
          },
        }),
  ...
  F = async (e, t) => {
    J(e.method, "Expected request method property to be set");
    let r;
...
class P extends Bx {
  constructor({
    ...
    if (
      ...
      ((this.ready = this._resolveRegionFromConfig(s).then((R) => {
        ((this.awsRegion = R),
          (this.baseURL = `https://aws-external-anthropic.${R}.api.aws`));
      })),
        this.ready.catch(() => {}));
  }
  ...
  validateHeaders() {}
  backendMiddleware() {
    return [async (e, t) => t(await X(this, _, "m", D).call(this, e))];
  }
}
((_ = new WeakSet()),
  (D = async function (t) {
