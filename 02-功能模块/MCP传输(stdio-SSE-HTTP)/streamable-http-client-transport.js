...
function createFetchWithInit(e = fetch, t) {
  ...
  return async (r, s) => {
    let a = {
      ...
      ...s,
      headers: s?.headers
        ? { ...headersToRecord(t.headers), ...headersToRecord(s.headers) }
        : t.headers,
    };
    return e(r, a);
  };
  ...
  async _startOrAuthSse(e) {
    ...
    try {
      ...
      if (!s.ok) {
        ...
        throw new StreamableHTTPError(s.status, `Failed to open SSE stream: ${s.statusText}`);
      }
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            this.onerror?.(
              Error(
                `Failed to reconnect: ${o instanceof Error ? o.message : String(o)}`,
              ),
            );
          }
      }
  ...
  async send(e, t) {
    try {
      ...
      if (!n.ok) {
        ...
        throw new StreamableHTTPError(n.status, `Error POSTing to endpoint: ${i}`);
      }
      if (
        ((this._hasCompletedAuthFlow = !1),
      ...
      let l =
          (Array.isArray(e) ? e : [e]).filter(
            (i) => "method" in i && "id" in i && i.id !== void 0,
          ).length > 0,
      ...
      if (l)
        if (_?.includes("text/event-stream"))
          ...
          let i = await n.json(),
            o = Array.isArray(i) ? i.map((c) => GR.parse(c)) : [GR.parse(i)];
          for (let c of o) this.onmessage?.(c);
        } else
          throw (
            await n.body?.cancel(),
            new StreamableHTTPError(-1, `Unexpected content type: ${_}`)
          );
  ...
  async terminateSession() {
    ...
    try {
      ...
      if ((await r.body?.cancel(), !r.ok && r.status !== 405))
        throw new StreamableHTTPError(r.status, `Failed to terminate session: ${r.statusText}`);
...
export { headersToRecord, createFetchWithInit, StreamableHTTPError, StreamableHTTPClientTransport };
