  ...
  async _startOrAuthSse(e) {
    ...
    try {
      ...
      if (!s.ok) {
        ...
        throw new _9(s.status, `Failed to open SSE stream: ${s.statusText}`);
      }
  ...
  async send(e, t) {
    try {
      ...
      if (!n.ok) {
        ...
        if (n.status === 403 && this._authProvider) {
          ...
          if (h === "insufficient_scope") {
            ...
            if (this._lastUpscopingHeader === f)
              throw new _9(403, "Server returned 403 after trying upscoping");
            if (c) this._scope = c;
        ...
        throw new _9(n.status, `Error POSTing to endpoint: ${i}`);
      }
      if (
        ((this._hasCompletedAuthFlow = !1),
      ...
      if (l)
        if (_?.includes("text/event-stream"))
          ...
          throw (
            await n.body?.cancel(),
            new _9(-1, `Unexpected content type: ${_}`)
          );
  ...
  async terminateSession() {
    ...
    try {
      ...
      if ((await r.body?.cancel(), !r.ok && r.status !== 405))
        throw new _9(r.status, `Failed to terminate session: ${r.statusText}`);
