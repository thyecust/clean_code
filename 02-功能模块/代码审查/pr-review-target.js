...
function d(n) {
  ...
  if (
    e.protocol !== "https:" ||
    normalizeHostname(e.hostname) !== GITHUB_HOST ||
    e.username !== "" ||
    e.password !== "" ||
    e.port !== "" ||
    e.search !== "" ||
...
var f = 8,
  m = [],
  makePrReviewTargetsStore = defineStoreField("prReviewTargets", m);
function recordPrReviewTarget(n, e) {
  let o = getCwd(),
    a = String(K()),
...
async function h(n, e) {
  ...
  let { code: o, stdout: a } = await execFileNoThrowWithCwd(
    "gh",
...
export { makePrReviewTargetsStore, recordPrReviewTarget, verifyPrReviewPublishTarget, getCurrentIsoTimestamp };
