...
var Y = "/api/frame/db/agent",
  ...
  ARTIFACT_PATH_SEGMENT_RE = new RegExp(`^${L}$`),
  ARTIFACT_COLLECTION_PATH_RE = new RegExp(`^${L}(?:/${L}){0,14}$`);
function isCollectionPath(e) {
  return (countOccurrences(e, "/") + 1) % 2 === 1;
}
function explainInvalidCollectionPath(e) {
  let r = (i) =>
...
var MAX_DOCUMENT_PATH_LENGTH = 1000,
  ARTIFACT_DB_READ_OPS = ["get", "list", "query"],
  ARTIFACT_DB_WRITE_OPS = ["set", "update", "delete"],
  MAX_DOCUMENT_BYTES = 262144,
  MAX_DOCUMENT_FILE_BYTES = 4 * MAX_DOCUMENT_BYTES,
  ue = 4194304,
  N = 16384,
  MAX_QUERY_LIMIT = 1000,
  MAX_BATCH_WRITE_OPS = 50,
  MAX_BATCH_REQUEST_BYTES = 1048576,
  fe = new Map([
    ["not_found", "not_found"],
...
async function W(e, r, t, c, o, u, i) {
  ...
  if (!d.success || !ARTIFACT_PATH_SEGMENT_RE.test(d.data.id))
    return {
      ok: !1,
...
async function runArtifactDbRead(e, r, t, c, o) {
  ...
  if (!ARTIFACT_COLLECTION_PATH_RE.test(s) || (d !== void 0 && !ARTIFACT_PATH_SEGMENT_RE.test(d)))
    return (
  ...
  if (!isCollectionPath(s))
    return (
      logFeatureBad("artifact_db_read", "parity"),
      {
        kind: "error",
        code: "invalid_argument",
        message: explainInvalidCollectionPath(s),
        reason: "parity",
      }
    );
  if (i === "get" && d === void 0)
  ...
  if (w.length > MAX_DOCUMENT_PATH_LENGTH)
    return (
      logFeatureBad("artifact_db_read", "path_too_long"),
      {
        kind: "error",
        code: "invalid_argument",
        message: `the composed document path is ${w.length} bytes \u2014 the limit is ${MAX_DOCUMENT_PATH_LENGTH}`,
...
async function se(e, r, t = "") {
  ...
  if (!ARTIFACT_COLLECTION_PATH_RE.test(c) || !ARTIFACT_PATH_SEGMENT_RE.test(o))
    return {
      ok: !1,
      result: E(
        "invalid_segment",
  ...
  if (!isCollectionPath(c)) return { ok: !1, result: E("parity", explainInvalidCollectionPath(c), t) };
  let u = re(c, o);
  if (u !== null) {
    let n = await r();
    if (!n.ok) return n;
  ...
  let i = `${c}/${o}`;
  if (i.length > MAX_DOCUMENT_PATH_LENGTH)
    return {
      ok: !1,
      result: E(
        "path_too_long",
        `the composed document path is ${i.length} bytes \u2014 the limit is ${MAX_DOCUMENT_PATH_LENGTH}`,
        t,
      ),
    };
  return { ok: !0, path: i };
...
async function runArtifactDbWrite(e, r, t, c, o) {
...
async function runArtifactDbBatchWrite(e, r, t, c, o) {
  let { slug: u, ops: i } = r;
  if (!ARTIFACT_SLUG_RE.test(u))
    return E("invalid_slug", "not a valid artifact id", "batch_");
  if (i.length === 0 || i.length > MAX_BATCH_WRITE_OPS)
    return E(
      "batch_size",
      `a batch takes 1-${MAX_BATCH_WRITE_OPS} writes \u2014 ${i.length} were given`,
      "batch_",
    );
  let n,
  ...
  let w = {
    ...
    h;
  try {
    h = Buffer.byteLength(jsonStringify({ slug: u, verb: "batch", args: w }), "utf8");
  } catch {
    return E(
      "data_invalid",
  ...
  if (h > MAX_BATCH_REQUEST_BYTES)
    return E(
      "batch_too_large",
      `the batch serializes to ${h} bytes \u2014 the limit for one request is ${MAX_BATCH_REQUEST_BYTES}; split it into smaller batches`,
      "batch_",
    );
  let v = batchSupportStore.of(e);
...
function Me(e, r, t) {
  ...
  return isDefinitiveWriteFailure(e)
...
function isDefinitiveWriteFailure(e) {
