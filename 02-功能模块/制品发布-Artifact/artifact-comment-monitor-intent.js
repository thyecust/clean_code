...
function setArtifactDurableRegistrySink(e) {
  getArtifactState().durable.registrySink = e;
}
function resetArtifactDurableRegistryPublished() {
  getArtifactState().durable.registryPublished = st;
}
function publishArtifactDurableRegistry() {
...
function parseArtifactDurableWatches(e) {
...
function resolveOwnProcStart() {
  return ownProcStartAsync();
}
function parseArtifactCommentMonitorIntent(e, t) {
  let n = dt(),
...
function readArtifactCommentMonitorRecords(e) {
  let t = C(e.storageV5),
...
function getTornArtifactCommentMonitorStops() {
  return getArtifactState().commentMonitorIntent.tornStops;
}
function getArtifactCommentMonitorStorage() {
  return getArtifactState().commentMonitorIntent.storageV5;
}
function ensureArtifactCommentMonitorState(e) {
  C(e?.storageV5);
}
function seedResumedArtifactCommentMonitor(e, t) {
  if (e === void 0 && !hasArtifactState()) return;
  let n = getArtifactState().commentMonitorIntent;
  ((n.earlySeed = e), (n.adoptPendingFor = K()), C(t?.storageV5));
}
function armArtifactCommentMonitor(e, t) {
  let n = C(t?.storageV5);
  if (getArtifactState().durable.stopLatches.isStopped(e)) return;
  let o = t?.title,
...
function ct(e, t) {
  ownProcStartAsync()
    .then(() => {
      if (!hasArtifactState() || !isActingAsBgJob()) return;
      let n = getArtifactState().commentMonitorIntent,
        o = n.bySlug.get(e);
      if (
        n.sid !== K() ||
...
function stopArtifactCommentMonitor(e, t) {
  let n = C(t?.storageV5),
    o = n.bySlug.get(e);
  (n.bySlug.set(e, {
    state: "stopped",
...
function getArtifactCommentMonitorState(e, t) {
  return C(t?.storageV5).bySlug.get(e)?.state;
}
function forgetArtifactCommentMonitors(e, t) {
  let n = C(t?.storageV5),
    o = !1,
...
function applyArtifactCommentMonitorStops(e, t) {
  let n = C(t?.storageV5),
    { stopLatches: o } = getArtifactState().durable;
  for (let [r, d] of e) {
...
function stopAllArmedArtifactCommentMonitors(e) {
  let t = C(e?.storageV5),
    { yieldedSlugs: n } = getArtifactState().wakes,
    o = Date.now(),
    r = !1;
...
function isArtifactCommentMonitorWired(e) {
...
function ut() {
  let e = getArtifactState().commentMonitorIntent,
    t = e.sid;
  if (t === null || t === K()) {
    e.leftWith = null;
    return;
  ...
  let n = new Set();
  for (let [o, r] of e.bySlug) if (r.state === "armed" && isArtifactCommentMonitorWired(o)) n.add(o);
  e.leftWith = {
    sid: t,
    traveling: n,
...
function B(e, t, n, o, r) {
  if (t === null) return;
  let d = getArtifactState().commentMonitorIntent,
    i = { path: t, line: Q(e, n) };
...
function P(e, t, n, o) {
  if (t === null) return;
  let r = getArtifactState().commentMonitorIntent,
    { storageV5: d } = r,
    i = Q(e, { ...n }),
...
function markArtifactCommentMonitorTraveling(e, t) {
  tt(C(t?.storageV5), e);
}
function tt(e, t) {
  let n = !1;
...
function C(e) {
  let t = getArtifactState().commentMonitorIntent;
  ...
  if (t.sid !== n) {
    let i = t.leftWith !== null && t.leftWith.sid === t.sid ? t.leftWith : null,
      u = (S) => (i !== null ? i.traveling.has(S) : isArtifactCommentMonitorWired(S));
    t.leftWith = null;
    let p = _(t),
      f = new Set(),
      l = 0;
    ...
    let { stopLatches: h } = getArtifactState().durable;
    for (let [S, w] of Object.entries(t.parked.get(n)?.line ?? {})) {
      ...
      if (
        ...
        M !== void 0 &&
        w.writtenAtMs <= M &&
        !isArtifactCommentMonitorWired(S)
      ) {
        r = !0;
        continue;
      }
  ...
  if (((t.earlySeed = void 0), d !== void 0)) {
    let i = !1,
      u = parseArtifactCommentMonitorIntent(d, {
        onLossy: () => {
          i = !0;
        },
        onTornStop: (p) => {
    ...
    if (u !== null) {
      ...
      let { stopLatches: p } = getArtifactState().durable,
        f = new Map();
      for (let [l, h] of u) {
        ...
        if (
          ...
          S !== void 0 &&
          h.writtenAtMs <= S &&
          !isArtifactCommentMonitorWired(l)
        ) {
          i = !0;
          continue;
        }
        let w = t.bySlug.get(l);
        if (t.tornStops.has(l) && w?.state === "armed" && isArtifactCommentMonitorWired(l)) {
...
function et() {
  let { commentMonitorIntent: e, wakes: t } = getArtifactState();
  if (
    !e.wroteCurrentLine ||
    e.sid !== K() ||
...
function A(e) {
  let t = C();
  (E(), publishArtifactDurableRegistry());
  let n = _(t),
    o = V(n);
...
export {
  setArtifactDurableRegistrySink,
  resetArtifactDurableRegistryPublished,
  publishArtifactDurableRegistry,
  parseArtifactDurableWatches,
  resolveOwnProcStart,
  parseArtifactCommentMonitorIntent,
  readArtifactCommentMonitorRecords,
  getTornArtifactCommentMonitorStops,
  getArtifactCommentMonitorStorage,
  ensureArtifactCommentMonitorState,
  seedResumedArtifactCommentMonitor,
  armArtifactCommentMonitor,
  stopArtifactCommentMonitor,
  getArtifactCommentMonitorState,
  forgetArtifactCommentMonitors,
  applyArtifactCommentMonitorStops,
  stopAllArmedArtifactCommentMonitors,
  isArtifactCommentMonitorWired,
  markArtifactCommentMonitorTraveling,
};
