...
function m(e) {
  let i = detectImageMediaType(e);
  if (i === "image/png")
    return e.length >= 8 && e.subarray(0, 8).equals(v) ? i : null;
...
var THUMBNAIL_DECLARATION_PARAGRAPH =
...
function parseThumbnailDeclarations(e, i) {
  ...
  if (t.badMedia !== void 0)
    a.problems.push(
      `a <link rel="artifact-thumbnail"> tag has media=${jsonStringify(t.badMedia)}; the only media value allowed is "(prefers-color-scheme: dark)" (for the dark-mode variant) \u2014 remove or fix the attribute`,
    );
...
function getDeclaredThumbnailHrefs(e) {
  return [e.light?.href, e.dark?.href].filter((i) => i !== void 0);
}
function filterDeclaredThumbnailsByRoot(e, i, t) {
...
async function loadDeclaredThumbnails(e, i, t, a) {
  if (e.problems.length > 0) throw h(e.problems.join("; "));
  let s = { skipped: [...e.skipped] };
  for (let r of ["light", "dark"]) {
    let n = e[r];
    if (n === void 0) continue;
    let l = jsonStringify(n.href);
    ...
    if (o !== void 0) {
      s[r] = w(
        n.href,
        typeof o.content === "string" ? Buffer.from(o.content) : o.content,
      );
      continue;
    }
...
function formatDeclaredThumbnailsSentence(e) {
  let i = e.map(
...
function buildRuntimeCapabilitiesParagraph(e) {
...
function buildStateKeepingPagesParagraph() {
...
function getCommentsUnavailableNote() {
...
var UPDATE_ARTIFACT_PARAGRAPH =
  ...
  FIND_ARTIFACTS_APP_NOTE = `If the user asks how to get back to their artifacts, the gallery at claude.ai/code/artifacts lists them.
...
function buildWatchRepublishesParagraph(e, i) {
  return i === "none"
...
var READ_BEFORE_PUBLISH_PARAGRAPH = `**Files you did not write**: Read the complete file before publishing it, even when asked not to ("it's personal", "no need to open it") \u2014 publishing distributes the content, and you must never distribute what you haven't seen. A request for privacy is a reason to read before publishing, not an exemption. If you cannot read it, do not publish it.`,
  ...
  f = `${CDN_ALLOWLIST_PARAGRAPH} **How to load a library**: \`<script src="https://cdnjs.cloudflare.com/ajax/libs/<lib>/<exact version>/<file>">\` \u2014 pick the UMD build, which defines a global (e.g. react/18.3.1/umd/react.production.min.js, then react-dom) \u2014 placed BEFORE any inline \`<script>\` that uses it; always pin an exact version. The viewer's sandbox also blocks any download the page starts itself \u2014 \`<a download>\` links (data:/blob: hrefs included) and script-driven saves are inert for viewers \u2014 so never offer a file through a plain link. Artifacts render mermaid diagrams natively \u2014 markdown via \`\`\`mermaid fences, HTML via \`<pre class="mermaid">\` blocks \u2014 no library needed, don't load one.
...
${buildWatchRepublishesParagraph(e, i)}
...
var L =
    "the Artifact tool's description lists the few other script hosts the CSP admits",
  N = "the page contract above lists the few other script hosts the CSP admits";
function prependPageContract(e) {
  let i = C();
...
function buildPageContractFallback() {
...
export {
  ...
  FIND_ARTIFACTS_TERMINAL_NOTE,
  buildWatchRepublishesParagraph,
  READ_BEFORE_PUBLISH_PARAGRAPH,
  CDN_ALLOWLIST_PARAGRAPH,
  RESPONSIVE_PARAGRAPH,
  THEME_AWARE_PARAGRAPH,
