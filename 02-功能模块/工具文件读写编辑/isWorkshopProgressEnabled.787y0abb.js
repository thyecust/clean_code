...
function P(t, r) {
  ...
  t.flushedAt = e;
  let o = deriveAuthoringProgress(t.raw, t.batch);
  if (!o) return;
  t.shown = !0;
...
function onToolUseStop(t) {
  ...
  a(() => {
    if ((r.delete(t), e.shown))
      (W3t.setSpinnerMessage(null), logFeatureOk("workshop_authoring_progress"));
  }, "stop");
}
function extractFilePaths(t) {
  let r = [];
  for (let e of t
    .slice(0, g)
...
function deriveAuthoringProgress(t, r = !1) {
  let e = -1;
  ...
  if (n > 0)
    return S.test(i)
      ? `drawing decision ${n} \u2014 ${truncateToWidth(i, A)}`
      : `drawing decision ${n}`;
...
export {
  ...
  extractFilePaths,
  isWorkshopProgressEnabled,
  onInputJsonDelta,
  onToolUseStart,
  onToolUseStop,
  resetAuthoringProgress,
};
