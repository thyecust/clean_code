...
async function recordUpdateResult(e, t) {
  if (isHoverRestEnabled() && t) {
    let i = await t.write(O(), jsonStringify(e), { mode: 438 & ~process.umask() });
    if (!i.ok)
      logForDebugging(`Failed to record update result: ${i.error.code}`, { level: "error" });
    return;
  }
  try {
    await getFileStorage().atomicWrite(T(), jsonStringify(e));
  } catch (i) {
    logForDebugging(`Failed to record update result: ${i}`, { level: "error" });
  }
}
async function readUpdateResult(e) {
  let t;
  if (isHoverRestEnabled() && e) {
    let i = await e.read([O()]);
    if (!i.ok)
      return (
        logForDebugging(`Failed to read update result: ${i.error.code}`, { level: "error" }),
        null
      );
    ...
    try {
      ...
      if (!W(i)) logForDebugging(`Failed to read update result: ${i}`, { level: "error" });
      return null;
    }
  try {
    let i = Q().safeParse(jsonParse(t));
...
class E {
  ...
  async ownsExecutable(e, t, i) {
    ...
    if (s.code === 0 && s.stdout)
      return (logForDebugging(`Detected ${t} installation: ${s.stdout.trim()}`), !0);
    return !1;
  }
  detectHomebrew() {
    ...
    if (t.includes("/Caskroom/"))
      return (logForDebugging(`Detected Homebrew cask installation: ${t}`), !0);
    return !1;
  }
  getHomebrewCaskName() {
  ...
  detectWinget() {
    ...
    for (let t of [
      ...
      if (t.test(e)) return (logForDebugging(`Detected winget installation: ${e}`), !0);
    return !1;
  }
  detectMise() {
    let e = this.sources.execPath();
    if (/[/\\]mise[/\\]installs[/\\]/i.test(e))
      return (logForDebugging(`Detected mise installation: ${e}`), !0);
    return !1;
  }
  detectAsdf() {
    let e = this.sources.execPath();
    if (/[/\\]\.?asdf[/\\]installs[/\\]/i.test(e))
      return (logForDebugging(`Detected asdf installation: ${e}`), !0);
    return !1;
  }
  getPackageManager() {
...
async function ce() {
  ...
  if (isRunningFromLocalInstall()) return "npm-local";
  if (
    [
...
async function pe() {
  ...
  if (s.code === 0 && s.stdout) {
    ...
    if (u) {
      let p = !1;
      try {
        if ((await realpath(d)).includes("/Caskroom/")) p = detectHomebrew();
      } catch {}
      if (!p) {
        let c = !1;
        for (let R of r) {
...
async function fe(e) {
  ...
  if (e === "native") {
    ...
    if (!me(o, d))
      if (getCurrentPlatform() === "windows") {
        ...
        let c = detectCurrentShell(),
          v = getShellConfigPaths()[c],
  ...
  if (e === "npm-global" && (await localInstallExists()))
    t.push({
...
async function getInstallationDiagnostics({ probeKeychain: e = !1, storageV5: t } = {}) {
  let i = await detectInstallType(),
    r =
      typeof {
        ISSUES_EXPLAINER:
          "report the issue at https://github.com/anthropics/claude-code/issues",
  ...
  let u = getGlobalConfig().installMethod || "not set",
    p = null;
  if (i === "npm-global") {
    if (((p = (await checkGlobalInstallPermissions()).hasPermissions), !p && !getAutoUpdaterDisabledReason()))
      o.push({
  ...
  let c = getRipgrepStatus(),
    ...
    v = i === "package-manager" ? await getPackageManager() : void 0,
    S = await readUpdateResult(t);
  return {
    installationType: i,
    version: r,
    installationPath: s,
...
export { recordUpdateResult, readUpdateResult, detectHomebrew, getHomebrewCaskName, getPackageManager, hasDetectedInstallType, detectInstallType, getInstallationDiagnostics };
