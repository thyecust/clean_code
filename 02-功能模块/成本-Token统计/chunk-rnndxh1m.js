...
function RUn(e) {
  let t = [],
    n = e.cases.map((r) => ({ ...r, name: Sn(r.name) })),
...
function S(e) {
  ...
  for (let n of e.runs) {
    ...
    if (n.error) return Sn(n.error);
  }
  if (t !== null) {
    let n = t.name,
      s = e.advisories?.find((d) => d.grader === n),
      a = e.runs.some((d) => d.graders.some((r) => r.name === n && r.passed));
    if (s !== void 0 && !a) return Sn(s.text);
  }
  return t ? Sn(`${t.name}: ${t.explanation}`) : null;
}
function o(e, t) {
  let n = te(e);
  return n > t ? `${e} ` : e + os(" ", t - n);
}
function Vit(e, t, n, s, a) {
  let d = e.reduce(
    (r, { report: l }) =>
      r +
...
export { vUn, hen, RUn, Vit, kUn, xUn, h0e };
