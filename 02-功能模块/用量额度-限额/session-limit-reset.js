...
import { isContinuableUsageLimitWall, getSubscriptionTier, sendAutoContinuationPrompt } from "./usage-limit-continuation.js";
...
function getSessionLimitResetState() {
  return b().state;
}
function _(e) {
  let t = b();
...
function subscribeToSessionLimitResetChanges(e) {
  return b().changed.subscribe(e);
}
function subscribeToSessionLimitResetEvents(e) {
...
function U(e) {
  ...
  if (!t.success)
    return (
      logForDebugging(`[juniper-tide] ignoring a malformed status block: ${t.error.message}`),
      null
    );
  let r = t.data,
    a = r.event_props;
...
async function R(e) {
  try {
    ...
    if (
      (logForDebugging(`[juniper-tide] status fetch failed: ${ge(t).message}`, {
        level: "warn",
      }),
...
async function E(e) {
  ...
  if (!t)
    return (
      logForDebugging("[juniper-tide] no OAuth organization; cannot claim"),
  ...
  try {
    ...
    if (!a.success)
      return (
        logForDebugging(`[juniper-tide] unreadable claim response: ${a.error.message}`, {
          level: "error",
        }),
    ...
    if (cc(r))
      logForDebugging(`[juniper-tide] claim failed: ${ge(r).message}`, { level: "warn" });
    else logError(ge(r));
    if (isAxiosError(r))
...
function x(e) {
  let t = getSessionLimitResetState();
...
function isSessionLimitResetAvailable(e, t = Date.now()) {
  let r = x(e);
  return (
...
function formatSessionLimitSpentLine(e, t = Date.now()) {
  let r = x(e);
  if (
    !isContinuableUsageLimitWall(e) ||
    r === void 0 ||
...
function handleContinuableSessionLimitWall(e, t) {
...
function C({
  ...
  let o = getSessionLimitResetState();
...
function logSessionLimitResetShown(e, t) {
...
function retrySessionLimitResetOnDialog(e, t) {
  if (k(e)) w(e, t, "dialog");
}
async function requestSessionLimitReset(e, t) {
  try {
    return await K(e, t);
  } catch (r) {
...
async function K(e, t) {
  ...
  if (!isSessionLimitResetAvailable(d) || d.resetsAt === void 0) {
    let f = formatSessionLimitSpentLine(d);
...
function te(e, t) {
  let r = getSessionLimitResetState();
...
function ie(e) {
  ...
  ((e.sessionSwitchSubscribed = !0),
    sc((t, r) => {
      if (isConversationResetSwitchReason(r)) e.continuableWallResetsAt = null;
    }));
}
export { getSessionLimitResetState, subscribeToSessionLimitResetChanges, subscribeToSessionLimitResetEvents, isSessionLimitResetAvailable, formatSessionLimitSpentLine, handleContinuableSessionLimitWall, logSessionLimitResetShown, retrySessionLimitResetOnDialog, requestSessionLimitReset };
