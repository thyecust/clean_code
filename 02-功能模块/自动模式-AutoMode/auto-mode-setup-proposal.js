...
async function proposeAutoModeSetup(o, s, t, i = gatherAutoModeRecon, a = sideQuery, e, k) {
  ...
  try {
    ...
    return (
      ...
      logForDebugging(`auto-mode-setup gather failed: ${l(d)}`, { level: "error" }),
      {
        ok: !1,
        code: "recon_failed",
        reason:
  ...
  let [c] = getThinkingBudgetDefaults(r),
    ...
    S = async (d) => {
      try {
        ...
        return (
          logForDebugging(`auto-mode-setup sideQuery failed: ${l(_)}`, { level: "error" }),
          {
            ok: !1,
            result: {
              ok: !1,
  ...
  let h = parseAutoModeProposal(u.text),
    A = !1;
  if (!h.ok && h.code === "parse_failed" && u.text.trim() !== "") {
    ...
    if (!d.ok) {
      ...
      let _ = parseAutoModeProposal(d.text);
      if (_.ok) ((h = _), (A = !0));
    }
  }
...
function parseAutoModeProposal(o) {
  let s = xt(bx(o), !1),
    t = O().safeParse(s);
  if (!t.success)
    return {
...
export { proposeAutoModeSetup, parseAutoModeProposal };
