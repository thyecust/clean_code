...
class v {
      ...
      this.socket.on("data", (l) => {
        ...
        while (this.responseBuffer.length >= 4) {
          ...
          try {
            ...
            if (H(r)) {
              if (
                (t.info(`[${e}] Received notification: ${r.method}`),
                this.notificationHandler)
              )
                this.notificationHandler(r);
            } else if (L(r))
              (t.info(`[${e}] Received tool response: ${r}`),
  ...
  scheduleReconnect() {
    ...
    if (this.reconnectAttempts > n) {
      (t.info(
        `[${e}] Giving up after ${n} attempts. Will retry on next tool call.`,
      ),
        (this.reconnectAttempts = 0));
      return;
    }
    ...
    if (this.reconnectAttempts <= this.maxReconnectAttempts)
      t.info(
        `[${e}] Reconnecting in ${Math.round(o)}ms (attempt ${this.reconnectAttempts})`,
      );
    else if (this.reconnectAttempts % 10 === 0)
      t.info(
        `[${e}] Still polling for native host (attempt ${this.reconnectAttempts})`,
      );
...
async function V(e) {
  ...
  try {
    ...
    if (
      n.isDirectory() &&
      (o === void 0 || (n.uid === o() && (n.mode & 511) === 448))
    )
      return t;
  } catch {}
  return ((w = void 0), E());
...
async function K(e, t) {
  if (!t.listConnectedExtensions) return (m.set(t, !0), null);
  if (m.get(t) && t.hasActiveSelection?.()) return null;
  m.delete(t);
  let n = await t.listConnectedExtensions(),
    o = n.map((r) => r.deviceId);
  if (n.length === 0) return null;
  if (n.length === 1) return (m.set(t, !0), null);
  ...
  try {
    ...
    let s = await t.callTool(
        "computer",
        {
          ...
          text: l,
          tabId: o,
          ...(c !== void 0 ? { expected_origin: c } : {}),
        },
        {
          ...n,
          permissionMode: "skip_all_permission_checks",
          onPermissionRequest: void 0,
        },
      ),
      r =
...
async function ne(e, t, n, o, l) {
  if (n !== "navigate" || (o.tabId !== null && o.tabId !== void 0))
    return { args: o };
...
async function oe(e, t, n, o, l) {
  ...
  if (
    (e.logger.silly(
      `[${e.serverName}] Received result from socket bridge: ${B(c)}`,
    ),
    c === null || c === void 0)
  )
        ...
        if (
          ...
          if (
            ...
            return {
              ...
              data: h.source.data,
              mimeType:
                "media_type" in h.source
                  ? h.source.media_type || "image/png"
                  : "image/png",
            };
        }
        if (typeof f === "object" && f !== null && "type" in f) return f;
  ...
  return (
    e.logger.warn(
      `[${e.serverName}] Unexpected result format from socket bridge`,
      c,
    ),
    {
...
async function ie(e, t, n) {
  ...
  if (
    !e.bridgeConfig ||
    !t.selectExtensionById ||
    !t.listConnectedExtensions ||
    !o
  )
    return {
      content: [
