...
import { initESM } from "../../01-核心基础设施/内嵌资源与模块互操作/chunk-2c9tjhwd.js";
class x {
  marshaller;
  serializer;
  deserializer;
          ...
          if (l.isStructSchema()) {
            ...
            for (let [s, v] of l.structIterator()) {
              ...
              if (((u = u || Boolean(g || w)), w)) {
                ...
                else if (v.isStringSchema())
                  c[s] = (this.serdeContext?.utf8Encoder ?? z.toUtf8)(n);
                else if (v.isStructSchema())
                  c[s] = await this.deserializer.read(v, n);
              } else if (g) {
                let b = t[e].headers[s]?.value;
                if (b != null)
                  if (v.isNumericSchema())
                    if (b && typeof b === "object" && "bytes" in b)
  ...
  writeEventBody(a, i, o) {
    ...
    return {
      body:
        typeof S === "string"
          ? (this.serdeContext?.utf8Decoder ?? z.fromUtf8)(S)
          : S,
      eventType: d,
