  ...
  H = (e, o) => {
    let t = e
      ? e
          ...
          .filter(Boolean)
      : [];
    for (let r of o.split(",").map((n) => n.trim()))
...
class AnthropicGoogleCloud extends Bx {
  ...
  constructor({
    ...
    project: o = l("ANTHROPIC_GOOGLE_CLOUD_PROJECT") ??
    ...
    if (!x)
      if (o) x = S(o, t, r);
      else if (f) throw new gn(`No project was given. ${D}`);
      else k = !0;
    super({
      baseURL: x ?? null,
      ...T,
    ...
    if (!k) this.ready = Promise.resolve();
      ...
      let h =
          _ ??
          s ??
          new A.GoogleAuth({ scopes: L, ...(i ? { authClient: i } : {}) }),
        v = t,
        N = r;
