...
var p = toESM(awsSdkCoreClientModule()),
  E = toESM(getPropertyProviderModule()),
  ...
  fromEnv = (e) => async () => {
    ...
    let o = process.env[AWS_ACCESS_KEY_ID_ENV_VAR],
      r = process.env[AWS_SECRET_ACCESS_KEY_ENV_VAR],
      n = process.env[u],
      s = process.env[_],
      t = process.env[C],
      d = process.env[w];
      ...
      [
        async () => {
          if (e.profile ?? process.env[m.ENV_PROFILE]) {
            if (process.env[AWS_ACCESS_KEY_ID_ENV_VAR] && process.env[AWS_SECRET_ACCESS_KEY_ENV_VAR]) {
              if (!v)
...
export { AWS_ACCESS_KEY_ID_ENV_VAR, AWS_SECRET_ACCESS_KEY_ENV_VAR, fromEnv, defaultProvider };
