...
async function retentionCleanupSkipReason(e) {
...
async function isRetentionCleanupSafe(e) {
  return (await retentionCleanupSkipReason(e)) === null;
}
function getCutoffDate(e) {
...
async function fe(e, t, r) {
  ...
  let o;
  try {
    let f = Fe().safeParse(jsonParse(a.content));
...
function isRetentionExemptionDisabled() {
...
function isCleanupQuietFsErrno(e) {
  return Rt(e) || Bp(e);
}
async function pe(e, t, r) {
  let a = E();
  try {
    let o = await getFsSurface().readdir(e);
    for (let f of o)
      try {
        if (Ne(f.name) < t)
    ...
    if (W(o));
    else if (isCleanupQuietFsErrno(o))
      logForDebugging(`cleanup readdir ${e} failed: ${o.code}`, { level: "error" });
    else logError(o);
  }
  return a;
}
async function je() {
  ...
  try {
    ...
    if (W(f));
    else if (isCleanupQuietFsErrno(f))
      logForDebugging(`cleanup mcp-logs scan failed: ${f.code}`, { level: "error" });
    else logError(f);
  }
  return o;
}
...
async function Be() {
  ...
  let w = isRetentionExemptionDisabled(),
    y = xe(),
    p = null,
    g = w
      ? void 0
...
async function v(e, t, r = !0, a) {
  let o = getCutoffDate(a),
    f = E();
  if (o === null) return f;
  let w = a === void 0 ? o : (getCutoffDate() ?? o),
...
async function Le() {
  ...
  try {
    ...
    if (!W(a))
      (logForDebugging(`Failed to clean up HFI auth file: ${a}`, { level: "error" }),
        e.errors++);
  }
  return e;
}
async function Ue() {
  ...
  try {
    ...
    if (!W(a))
      if ((e.errors++, isCleanupQuietFsErrno(a)))
        logForDebugging(`cleanup team-discovery-cache failed: ${a.code}`, { level: "error" });
      else logError(a);
  }
  return e;
}
...
async function He() {
  ...
  try {
    ...
    if (!W(a))
      if ((e.errors++, isCleanupQuietFsErrno(a)))
        logForDebugging(`cleanup mcp-auth-cache failed: ${a.code}`, { level: "error" });
      else logError(a);
  }
  return e;
}
async function Ye() {
  ...
  try {
    ...
    if (!W(a))
      if ((e.errors++, isCleanupQuietFsErrno(a)))
        logForDebugging(`cleanup device-unbound-creates failed: ${a.code}`, {
          level: "error",
        });
      else logError(a);
  }
...
async function Xe(e) {
  ...
  try {
    ...
    return (
      logForDebugging(`History retention prune failed: ${p}`, { level: "error" }),
      { entriesPruned: 0, errors: 1 }
    );
  }
  ...
  try {
    w = await Cs(t, {
      ...
      onCompromised: (p) => {
        ((y = !0),
          logForDebugging(`History retention lock compromised: ${p}`, { level: "error" }));
      },
    });
  } catch (p) {
    if (W(p)) return { entriesPruned: 0, errors: 0 };
...
async function ge(e, t) {
  ...
  let w;
  try {
    w = await getFsSurface().readdir(o);
  } catch {
    return r;
  }
  for (let y of w) {
...
async function Pt(e) {
  ...
  let a = isRetentionExemptionDisabled(),
    o = getCutoffDate(),
    f = new Set();
...
async function healRefusedSentinelLeaf(e) {
  ...
  logForDebugging(
    `.last-cleanup: sentinel is not a regular file (${describeStorageError(t.error)}) \u2014 ${Tt(r)}; rewriting`,
    { level: "warn" },
  );
}
var xt = [
...
async function reapStaleHousekeepingStagingFiles(e, t) {
...
async function cleanupOldMessageFilesInBackground(e) {
  await cleanupStaleImageCacheDirs();
  let t = await retentionCleanupSkipReason(e),
...
export { retentionCleanupSkipReason, isRetentionCleanupSafe, getCutoffDate, isRetentionExemptionDisabled, isCleanupQuietFsErrno, healRefusedSentinelLeaf, reapStaleHousekeepingStagingFiles, cleanupOldMessageFilesInBackground };
