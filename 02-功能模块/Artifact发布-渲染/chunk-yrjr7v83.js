...
var Lon =
  ...
  Mon = `If the user asks how to get back to their artifacts, the gallery at claude.ai/code/artifacts lists them.
...
var $on = `**Files you did not write**: Read the complete file before publishing it, even when asked not to ("it's personal", "no need to open it") \u2014 publishing distributes the content, and you must never distribute what you haven't seen. A request for privacy is a reason to read before publishing, not an exemption. If you cannot read it, do not publish it.`,
  ...
  f = `${Uon} **How to load a library**: \`<script src="https://cdnjs.cloudflare.com/ajax/libs/<lib>/<exact version>/<file>">\` \u2014 pick the UMD build, which defines a global (e.g. react/18.3.1/umd/react.production.min.js, then react-dom) \u2014 placed BEFORE any inline \`<script>\` that uses it; always pin an exact version. The viewer's sandbox also blocks any download the page starts itself \u2014 \`<a download>\` links (data:/blob: hrefs included) and script-driven saves are inert for viewers \u2014 so never offer a file through a plain link. Artifacts render mermaid diagrams natively \u2014 markdown via \`\`\`mermaid fences, HTML via \`<pre class="mermaid">\` blocks \u2014 no library needed, don't load one.
