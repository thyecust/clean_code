...
function isAdoptableSessionId(t, e = {}) {
  return e.acceptCustomIds ? j1(t) : ft(t);
}
function resolveSessionAdoption(t, e, i = {}) {
  if (e) return { adoptedSessionId: null, effectiveFork: !0 };
  if (t === void 0) return { adoptedSessionId: null, effectiveFork: !1 };
  if (isAdoptableSessionId(t, i)) return { adoptedSessionId: t, effectiveFork: !1 };
  return (
    logForDebugging(
...
function X(t) {
  return new SymlinkReadRefusedError(
...
async function openApprovedPathForRead(t, e, i) {
  let o = new Set(e);
    ...
    if (u.isSymlink || u.resolvedPath !== t) {
      if (o.has(u.resolvedPath) && it(u.resolvedPath, r)) {
        let y = r === "macos" ? await a(u.resolvedPath, !1) : await a(t, !0),
          v = resolvePathInfo(getFsSurface(), t);
  ...
  try {
    ...
    if (m === w) {
      for (let v of expandPathAliases(t)) if (!o.has(v)) throw X(t);
    }
    let y = k;
...
var Dt = 3;
function C(t) {
  return new SymlinkWriteRefusedError(
...
async function pinWriteTarget(t, e, i) {
  for (let o = 1; ; o++)
    try {
...
async function Vt(t, e, i) {
  ...
  for (;;)
    try {
      ((et = s === "macos" ? m(D) : D), (N = await openNoSymlinkTraversal(et, z, s)));
      break;
    } catch (p) {
      let O = A(p),
        d = L(D),
...
function rethrowWithUserFacingPath(t, e, i) {
...
`
  ...
  async #g() {
    try {
      ...
      if (!this.#i)
        ((this.#i = !0),
          logForDebugging(`Task output drain failed (will retry once): ${t}`, {
            level: "error",
          }));
      if (this.#t.length > 0)
        try {
  ...
  #m(t) {
    ...
    if (!this.#a.has(o))
      switch ((this.#a.add(o), i)) {
        case "exhaustion":
          logForDebugging(`Task output drain retry failed (${e}): ${t}`, { level: "error" });
          break;
        case "unexpected":
          logError(t);
          break;
...
async function getTaskOutputDelta(t, e, i = Bt) {
  try {
    ...
    let r;
    try {
      r = await readBytesAtOffset(o, e, i);
    } finally {
      await o.close();
    }
    ...
    if (r && WW.has(r))
      logForDebugging(`getTaskOutputDelta failed (${r}): ${o}`, { level: "error" });
    else logError(o);
...
${r}`;
    ...
    if (o === "ENOENT") return "";
    if (o && WW.has(o))
      logForDebugging(`getTaskOutput failed (${o}): ${i}`, { level: "error" });
    else logError(i);
    return "";
  }
}
async function getTaskOutputSize(t) {
  try {
    ...
    if (i && WW.has(i))
      logForDebugging(`getTaskOutputSize failed (${i}): ${e}`, { level: "error" });
    else logError(e);
    return 0;
  }
}
async function repointTaskOutputSymlinks(t, e) {
  ...
  for (let s of o) {
    ...
    try {
      ...
      if (u !== "ENOENT")
        if (u && WW.has(u))
          logForDebugging(`repointTaskOutputSymlinks readdir failed (${u}): ${a}`, {
            level: "error",
          });
        else logError(a);
      continue;
...
async function tailTaskOutput(t, e) {
  ...
  try {
    return await readTailBytes(i, e);
  } finally {
    await i.close();
  }
}
    ...
    (async () => {
      try {
        ...
        if ((r && WW.has(r)) || r === "EROFS")
          logForDebugging(`initTaskOutputAsSymlink failed (${r}): ${o}`, { level: "error" });
        else logError(o);
...
export {
  isAdoptableSessionId,
  resolveSessionAdoption,
  openNoSymlinkTraversal,
  openApprovedPathForRead,
  pinWriteTarget,
  rethrowWithUserFacingPath,
