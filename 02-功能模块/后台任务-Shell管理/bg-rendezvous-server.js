...
import { getPromptInputStore, setPromptInputValue } from "../../01-核心基础设施/核心工具-未归类/prompt-input-store.js";
...
import { markAttached, markDetached } from "../../01-核心基础设施/核心工具-未归类/attach-state-tracking.js";
import { getInkInstanceRegistry } from "../多会话视图-Fleet/ink-instance-registry.js";
import { hasEarlyInput, seedEarlyInput } from "../../01-核心基础设施/核心工具-未归类/early-input-capture.js";
import { getDraftMode, getDraftValue } from "../../01-核心基础设施/核心工具-未归类/bash-mode-draft-text.js";
import { setSystemTheme } from "../../01-核心基础设施/UI组件-TUI/theme-resolution.js";
...
async function se(e) {
  ...
  let r = re(t);
  if (r === null)
    (logForDebugging("[bg] persisted prompt stash unreadable or empty", { level: "warn" }),
  ...
  constructor(e, t, r, o, i) {
    ((this.authToken = t),
      ...
      this.server.on("error", (d) =>
        logForDebugging(`[bg-rv] server error: ${String(d)}`, { level: "warn" }),
      ),
...
async function startRendezvousServer(e) {
  ...
  let R = r.server;
  registerCleanup(() => R.promptStash.flush());
}
function sendRv(e) {
...
function le(e) {
  if (C(G(), e.text)) {
    logForDebugging(`[bg-rv] peer reply answered question: ${e.text.slice(0, 80)}`);
    return;
  }
  ...
  (enqueueCommand({
    ...
    logForDebugging(`[bg-rv] enqueued reply: ${e.text.slice(0, 80)}`));
}
...
export { registerPeerReplyResolver, startRendezvousServer, sendRv, disarmStartupWedgeWatchdog, markStartupDialogBlocked, clearStartupDialogBlocked, markCommandParkBlocked, clearCommandParkBlocked, markReplayNoOp };
