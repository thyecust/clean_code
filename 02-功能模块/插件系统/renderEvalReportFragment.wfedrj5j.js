...
function k(e) {
  return e.replace(Z, "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
}
function T(e) {
    ...
    renderer: {
      ...
      text(a) {
        ...
        if (a.type === "text" && a.escaped) {
          let n = a.text.match(J);
          if (n) return n[0] + k(a.text.slice(n[0].length));
        }
        return k(a.text);
      },
      link(a) {
...
function le(e) {
  let s = e.passed
    ...
    a =
      e.weight !== 1
        ? `<span class="muted">\xD7${r(String(e.weight))}</span>`
        : "",
    ...
    i =
      e.judgeVotes && e.judgeVotes.length > 1
        ...
        : "",
    l = e.evidence
      ? `<div class="kv"><span>Evidence (what the judge was shown; for an image, a description of what was sent)</span></div><pre class="evidence">${r(he(e.evidence))}</pre>`
      : "";
...
${(e.advisories ?? []).map((o) => `<div class="note">\u26A0 ${r(o)}</div>`)
  .join(`
`)}
${d}
${l}
