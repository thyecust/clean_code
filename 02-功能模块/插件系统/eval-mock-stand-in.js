...
function formatForDisplay(e) {
  return escapeNonPrintableAscii(jsonStringify(B(e)));
}
function formatValueForDisplay(e) {
  if (typeof e === "string") return formatForDisplay(e);
...
async function assertPathIsLocal(e, t, r, o = []) {
  ...
  let p = await resolveSymlinkAncestry(fsSurface, n, {
    anchors: a,
    surfaceNetworkRaw: !0,
    unreadableAncestry: "unverified",
  });
  if (p === UNVERIFIED_ANCESTRY_SENTINEL)
    throw new R(
...
function findExpectViolation(e, t, r = "") {
...
function ne(e, t, r, o) {
  ...
  if (typeof t === "object" && t !== null) return findExpectViolation(o, t, r);
  let n = G(t);
  if (n !== null) {
    let i =
        typeof e === "string"
...
async function renderPromptTemplate(e, t, r, o, n = Number.POSITIVE_INFINITY) {
  let i = null,
    a = [],
    p = 0;
  for (let y of e.matchAll(
    ...
    if (w.startsWith("input.")) {
      let g = j(t, w.slice(6)),
        x = g === void 0 ? "" : typeof g === "string" ? g : jsonStringify(g);
      a.push(
        x.length > n
...
var EVAL_ABORTED_BY_MOCK_MESSAGE = "eval aborted by mock",
  MOCK_AGENT_RESPONDER_FAILED_MESSAGE = "mock agent responder failed";
...
async function V(e, t, r) {
  try {
    let o = await me(
      e.socketPath,
      `${jsonStringify({ token: e.token, ...t })}
`,
      r,
    );
    return pe().parse(jsonParse(o));
  } catch (o) {
    return (
...
`),
      {
        verdict: "abort",
        text: `${MOCK_AGENT_RESPONDER_FAILED_MESSAGE} (harness_unreachable) \u2014 see the eval debug log`,
      }
    );
  }
}
...
var he = createLazyValue(() => {
  ...
  Y = 1048576;
async function serveMockStandIn(e, t) {
  ...
  if (!(await Z(o.callLogPath, jsonStringify({ ready: o.nonce, server: o.server }))))
    throw Error(
...
function be(e) {
  ...
  async function a(p) {
    ...
    if (!(await Z(e.callLogPath, jsonStringify(h))) && f.verdict === "abort")
      process.stderr
      ...
      case "abort":
        return {
          content: [
            {
              type: "text",
              text: `${EVAL_ABORTED_BY_MOCK_MESSAGE} ${e.nonce}: ${e.server}/${d} \u2014 ${f.text}`,
            },
          ],
          isError: !0,
        };
...
async function ye(e, t, r, o = "") {
  if (e.expect !== null) {
    let n = findExpectViolation(t, e.expect);
    if (n !== null)
  ...
  switch (e.kind) {
    ...
    case "fixed": {
      let n = await renderPromptTemplate(e.body, t, e.baseDir, readMockFixtureFile);
...
var MAX_INTERPOLATED_TEXT_CHARS = 16384,
  I = 16384;
function K(e) {
...
var xe = "| ",
  ke = new RegExp(
    `^(?:${EVAL_ABORTED_BY_MOCK_MESSAGE.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}|MCP server "|MCP error -32000: Connection closed)`,
  );
function Se(e) {
  return he().parse(jsonParse(e));
}
var Ee = 4194304;
async function ve(e) {
