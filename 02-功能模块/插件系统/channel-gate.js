...
import { CHANNEL_TAG } from "../../01-核心基础设施/提示词-SystemPrompt/chunk-27ncq5fr.js";
...
var ChannelMessageNotificationSchema = createLazyValue(() =>
  ...
  ChannelPermissionNotificationSchema = createLazyValue(() =>
    c({
      method: k(CHANNEL_PERMISSION_METHOD),
...
function wrapChannelMessage(e, r, i) {
...
function isChannelGateHardRevocation(e) {
  return (
...
function findChannelEntry(e, r) {
  let i = e.split(":");
  return r.find((o) =>
    o.kind === "server" ? e === o.name : i[0] === "plugin" && i[1] === o.name,
  );
}
function gateChannelServer(e, r, i, o) {
  if (!hasChannelCapability(r))
    return {
      action: "skip",
      kind: "capability",
  ...
  if (t.kind === "plugin") {
    ...
    if (!t.dev) {
      let { entries: u, source: l } = getEffectiveChannelAllowlist(a?.allowedChannelPlugins);
      if (
...
export { ChannelMessageNotificationSchema, CHANNEL_PERMISSION_METHOD, ChannelPermissionNotificationSchema, CHANNEL_PERMISSION_REQUEST_METHOD, wrapChannelMessage, getEffectiveChannelAllowlist, isChannelGateHardRevocation, isChannelsPolicyBlocked, findChannelEntry, gateChannelServer };
