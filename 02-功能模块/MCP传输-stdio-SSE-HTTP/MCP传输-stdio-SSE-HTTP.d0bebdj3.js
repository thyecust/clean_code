...
class WebStandardStreamableHTTPServerTransport {
  constructor(e = {}) {
    ((this._started = !1),
      ...
      (this._enableDnsRebindingProtection =
        e.enableDnsRebindingProtection ?? !1),
  ...
  createJsonErrorResponse(e, t, s, i) {
    ...
    return new Response(
      JSON.stringify({ jsonrpc: "2.0", error: r, id: null }),
      {
        status: e,
  ...
  validateRequestHeaders(e) {
    ...
    if (this._allowedHosts && this._allowedHosts.length > 0) {
      ...
      if (!t || !this._allowedHosts.includes(t)) {
        ...
        return (
          this.onerror?.(Error(s)),
          this.createJsonErrorResponse(403, -32000, s)
        );
      }
    }
    ...
    try {
      ...
      if (this._eventStore.getStreamIdForEventId) {
        ...
        if (this._streamMapping.get(t) !== void 0)
          return (
            this.onerror?.(
              Error("Conflict: Stream already has an active connection"),
            ),
            this.createJsonErrorResponse(
              409,
              -32000,
...
`),
  ...
  async handlePostRequest(e, t) {
    try {
      ...
      let o;
      try {
        if (Array.isArray(n)) o = n.map((d) => GR.parse(d));
        else o = [GR.parse(n)];
      } catch {
        return (
      ...
      return (
        this.onerror?.(s),
        this.createJsonErrorResponse(400, -32700, "Parse error", {
          data: String(s),
        })
      );
    }
  ...
  async send(e, t) {
    ...
    if (s === void 0) {
      ...
      let n;
      if (this._eventStore)
        n = await this._eventStore.storeEvent(this._standaloneSseStreamId, e);
      let o = this._streamMapping.get(this._standaloneSseStreamId);
      if (o === void 0) return;
      if (o.controller && o.encoder)
        this.writeSSEEvent(o.controller, o.encoder, e, n);
      return;
    }
    let i = this._requestToStreamMapping.get(s);
    if (!i)
      throw Error(`No connection established for request ID: ${String(s)}`);
    ...
    if (isJSONRPCResultResponse(e) || isJSONRPCErrorResponse(e)) {
      ...
      if (n.every((a) => this._requestResponseMap.has(a))) {
        ...
        if (this._enableJsonResponse && r.resolveJson) {
          ...
          if (l.length === 1)
            r.resolveJson(
              new Response(JSON.stringify(l[0]), { status: 200, headers: a }),
            );
          else
            r.resolveJson(
              new Response(JSON.stringify(l), { status: 200, headers: a }),
            );
        } else r.cleanup();
        for (let a of n)
          (this._requestResponseMap.delete(a),
            this._requestToStreamMapping.delete(a));
      }
    }
  }
}
