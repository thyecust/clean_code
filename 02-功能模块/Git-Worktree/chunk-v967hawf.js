...
async function Z(e) {
  ...
  let o = i === void 0 ? await Kan(r) : { kind: "none" };
  if (o.kind === "refused")
    return t(
...
var et = 2,
  V = 256;
async function t3n(e, { maxBytes: t }) {
...
async function Kan(e) {
  let t = await Kbe(H(e, "objects", "info", "alternates"));
...
var ce = 4096;
async function Kbe(e, { firstBytes: t } = {}) {
  try {
...
function Jan(e, { maxHeaderBytes: t = bt } = {}) {
...
function Qan(e, { refNames: t, maxPrerequisites: r = kt }) {
  let i = Jan(e);
...
`,
  ...
  if (v.exitCode !== 0 || v.content === null)
    return S("bundle_create", Mv("pack-objects", v));
...
async function Pt(e, t) {
  let r = await on(e, ["rev-list", "--objects", "--boundary", ...t], {
    maxBuffer: U,
  });
  if (r.exitCode === 0) {
    let s = Ie(r.stdout);
  ...
  let i = await on(e, ["rev-list", "--boundary", ...t], { maxBuffer: U });
  return i.exitCode === 0
...
async function Gt(e, t) {
  let r = await on(e, ["rev-list", "--objects", "--disk-usage", ...t]),
    i = r.stdout.trim();
...
async function Nt(e, t) {
  let r;
  try {
    let s = await dOe(e, Math.max(1048576, 128 * t));
    ...
    i
      ...
      .map(async (o) => {
        ...
        try {
          ...
          if (!W(c))
            n("dir-sync: could not sweep a stale sync file (non-fatal)");
        }
      }),
  );
}
...
`,
          ...
          return (
            n("dir-sync: a delivered-ids record could not be read"),
            null
          );
        }
      }),
...
async function Ut(e, t, r, i) {
  ...
  let o = await on(e, ["cat-file", "--batch-check=%(objecttype)"], {
      env: i,
      input: s
        .map(
          (d) => `${d}
