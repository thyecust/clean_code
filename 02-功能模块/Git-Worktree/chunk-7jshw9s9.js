...
async function Dan({
  ...
  try {
    ...
    return {
      probedAtMs: a,
      durationMs: Date.now() - a,
      verdict: _(_ze(o, u.signal, "threw")),
    };
  } finally {
    u.cleanup();
  }
...
async function q(e, i, r) {
  let t = await Kce(z(e, `refs/remotes/${i}/${r}`), e.signal);
  ...
  if (t.remoteSha === null)
    return {
      ...
      diverged: !0,
      remote: null,
      fallback: await Kce(Z(e, t, i), e.signal),
    };
  let o = await Kce(L(e, t, t.remoteSha), e.signal);
...
async function z(e, i) {
  let [r, t, o, a, d, u] = await Promise.all([
    Jb(e, ["symbolic-ref", "-q", "HEAD"]),
    Jb(e, ["rev-parse", "-q", "--verify", "HEAD"]),
    Jb(e, ["show-ref", "--verify", "--hash", i]),
    Jb(
      e,
...
`,
  ...
  let p = await Promise.all(
      v.filter((b, C) => s[C]).map((b) => Jb(e, ["show-ref", "--exists", b])),
    ),
    f = o.stdout.trim();
  return {
    headSha: c,
...
async function L(e, { headSha: i, headRecords: r, realRoot: t }, o) {
  let { gitRoot: a } = e,
    ...
    [u, c, s] = await Promise.all([
      ...
      d ? Promise.resolve(null) : Jb(e, P(o)),
      d
        ? Promise.resolve(null)
        : Jb({ ...e, timeoutMs: Math.min(e.timeoutMs, Y) }, [
            "merge-base",
            i,
            o,
          ]),
...
async function Lan(e) {
  let [i, r] = await Promise.all([Jb(e, P("HEAD")), xk(e.gitRoot)]),
...
export { Oan, Dan, dFt, _ze, Jb, pFt, Lan, Kce };
