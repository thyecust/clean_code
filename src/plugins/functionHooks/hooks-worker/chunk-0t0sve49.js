...
import { escapeRegExp, truncateToCodeUnits } from "./string-utils.js";
...
function Is({
  ...
  return {
    runBelow: i,
    call: async (a, f) => {
      let c = assertNextArgumentIsRecord(a, e.name),
...
var aa = BUILTIN_HOOK_EVENT_NAMES.filter((e) => e !== "PreToolUse");
function De() {
...
function wa({ pluginName: e, live: t, unloaded: r, invoke: o, signalFrom: n }) {
  ...
  return E({
    ...
    sleep: (i, a = {}) => {
      let f, c;
      try {
        if (((f = s(i, "sleep")), r())) throw createEnvironmentUnloadedError(e);
        c = n(a.signal);
      } catch (u) {
      ...
      return new Promise((u, y) => {
        ...
        let S = E({
          cancel: () => {
            (clearTimeout(w), T(), y(createEnvironmentUnloadedError(e)));
          },
        });
        t?.add(S);
      });
...
async function Hf({ args: e, context: t, intoEnvironment: r, stamped: o }) {
  let { modulePath: n, pluginName: s, pluginRoot: p, source: i } = e,
    a = resolve(p),
    f = new Map(),
    c = new vm.SyntheticModule([], () => {}, { context: t, identifier: BUILTIN_MODULE_SPECIFIER }),
    m = Ko(e),
    d = Uo(e.links);
  ...
  function b(w, S) {
    let O = f.get(w);
    if (O) return O;
    let C = new vm.SourceTextModule(transpileHooksModuleSource(w, S), {
      context: t,
      identifier: w,
...
async function Qf(e, t, r = {}) {
  ...
  function dn(x, v) {
    if (p) throw createEnvironmentUnloadedError(o);
    try {
      return i(() => d(x, O(v)));
    } catch (A) {
      throw J(A);
