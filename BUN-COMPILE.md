...
本树共 **366 个真实调用点**，另有 11 处只在注释/字符串里被提及（不计），分布在 58 个文件。
...
即：**740 个文件（45%）只靠 `import.meta.require` 被引用**。直接 `--compile`，它们一个都不会进包。
...
| C-getter 惰性模块访问器 | 63 | **56** |
| C-real 真业务函数 | 100 | **69** |
| D 类字段 / 静态块 | 1 | 1 |
| E 简洁箭头体 | 39 | 39 |
| **合计** | **366** | **125** |

- **免费 163 处（44.5%）**；需要动函数签名的 203 处（55.5%），落在 **125 个不同函数**上。
- C-real 的 69 个函数体量：`<1KB` 37 个、`1–10KB` 25 个、`10–100KB` 5 个、`>100KB` **2 个**
...
它们位于全树最公共的一层。所以这条路的成本不在 366 个调用点本身。
