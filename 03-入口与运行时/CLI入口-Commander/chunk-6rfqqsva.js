...
import { S, we, Ln } from "../../01-核心基础设施/共享小工具-未细化/chunk-w76kejwn.js";
...
function CPn() {
  ...
  if (!R) return;
  let t = L();
  if ((t.advance(), V(), bW().mark(`${E}turn_start`), T))
...
function Br(t, { once: o = !1 } = {}) {
  ...
  if ((bW().mark(t), P)) r.push(process.memoryUsage());
  return !0;
}
function U({ memorySnapshots: t }) {
  if (!P) return "Startup profiling not enabled";
  let s = bW().getEntriesByType("mark");
  ...
  for (let [f, e] of s.entries())
    (r.push(iXt(e.startTime, e.startTime - _, e.name, t[f], 8, 7)),
      (_ = e.startTime));
  let c = s.at(-1);
  return (
    r.push(""),
    r.push(`Total startup time: ${QQ(c?.startTime ?? 0)}ms`),
    r.push("=".repeat(80)),
    r.join(`
`)
  );
...
function rt() {
  return q(be(), "startup-perf", `${K()}.txt`);
}
function ot() {
  return q(be(), "startup-perf", `${K()}.json`);
}
function W({ firstEmitPhases: t, startupContext: o }, { late: s }) {
  let _ = bW().getEntriesByType("mark");
