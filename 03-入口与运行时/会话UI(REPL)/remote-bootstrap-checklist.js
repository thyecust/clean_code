...
function RemoteBootstrapProgress() {
  let D = _(22),
    u = useAppStateSelector(re),
  ...
  if (D[14] !== k)
    ((q =
      k > 0
        ? `${k} ${pluralize(k, "message")} queued \xB7 sends once the session is ready`
...
function J(Re) {
  ...
  switch (s.status) {
    case "completed": {
      ...
      let z;
      if (T[6] !== m || T[7] !== i)
        ((z = e(ChecklistStepRow, { status: "completed", label: m, annotation: i })),
          (T[6] = m),
          (T[7] = i),
          (T[8] = z));
      else z = T[8];
    ...
    case "pending": {
      let i;
      if (T[21] !== m)
        ((i = e(ChecklistStepRow, { status: "pending", label: m })),
          (T[21] = m),
          (T[22] = i));
      else i = T[22];
      return i;
...
export { RemoteBootstrapProgress, ChecklistStepRow };
