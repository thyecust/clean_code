...
- **1431 个 js 文件**（另有 255 个随行资源），**165 个目录**，约 48 MB
...
当前全绿：1431 个文件 / 43.0 MB / 17,269 条相对依赖边（其中懒加载 301 条）0 失败；
...
- `03-入口与运行时/核心应用-Agent循环/核心应用-Agent循环.wmzgeczq.js` （7.59 MB，现归入「核心应用 / Agent 循环」）
...
- **一个 chunk ≠ 一个源文件。** 打包器把共用代码提升进了少数巨型 chunk，
  最典型的是 `03-入口与运行时/核心应用-Agent循环/核心应用-Agent循环.wmzgeczq.js`（7.59 MB、3128 个导出），
  它内部含工具定义、插件加载、SystemPrompt 等多个模块，但只能放在一个目录里。
  它的**依赖闭合前缀**（打包器提升进来的 CJS 库）已按上节的办法切成同目录的
  `bundled-cjs-libraries.js`；其余部分仍待在一个文件里。
