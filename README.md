...
- 本树有 **366 个调用点**（58 个文件），其中 **740 个文件（45%）只靠懒加载入边被引用** ——
  直接 compile 会集体丢失。

原版二进制没事，是因为它的 chunk 是作为**独立文件**存在于 `/$bunfs/root/` 的。
要改造的话代价不在调用点本身，而在 **56 个 CJS 互操作 getter**（模块加载层）。
...
- **不能 `bun build --compile`。** `import.meta.require` 不是打包器的边，
  740 个文件只靠它被引用；要改造成 `await import` 得先处理 async 传染。见 [`BUN-COMPILE.md`](BUN-COMPILE.md)。
