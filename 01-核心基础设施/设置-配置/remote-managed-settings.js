...
function registerManagedSettingsConsentRequester(e) {
...
function acquireConsentHandoffHold() {
  let e = F(),
...
function isConsentHandoffHeld() {
  return F().consentHandoffHolds.size > 0;
}
function isConsentHandoffRevealActive() {
  return F().consentHandoffRevealActive;
}
function subscribeConsentHandoffHolds(e) {
  return F().consentHandoffSignal.subscribe(e);
}
function setConsentNeededRelease(e) {
  let t = F();
  return (
...
async function oe(e, t, o) {
  ...
  if (a.replRequester)
    if (a.consentNeededRelease) {
      let w = null;
      try {
        ((w = acquireConsentHandoffHold()), await a.fireConsentNeededRelease());
...
async function ue(e) {
  ...
  if (isHoverRestEnabled() && e !== void 0) {
    ...
    try {
      ...
      return (logForDebugging(`Remote settings: Consent records unreadable - ${l(a)}`), x());
    }
    if (!o.ok)
      return (
        logForDebugging(`Remote settings: Consent records unreadable - ${describeStorageError(o.error)}`),
        x()
      );
...
async function ge(e, t, o) {
  try {
    ...
    let E = jsonStringify({ version: L, records: Object.fromEntries(P) });
    if (isHoverRestEnabled() && o !== void 0) {
      let C = await o.write(STORAGE_KEYS.state(ce), E, { mode: 384 });
      if (!C.ok)
        logForDebugging(`Remote settings: Failed to record org consent - ${describeStorageError(C.error)}`);
      return;
    }
    await getFileStorage().atomicWrite(de(), E, 384);
  } catch (r) {
    logForDebugging(`Remote settings: Failed to record org consent - ${l(r)}`);
  }
}
...
function ye() {
  setPolicyColdStartWaiter(async () => {
    if (shouldAwaitRemoteSettingsConfirmation()) await awaitRemoteSettingsFetchSettled();
    else if (isRemoteSettingsFetchPending()) await awaitRemoteSettingsLoaded();
  });
}
async function ot(e) {
  let t = ns();
    ...
    try {
      ...
      if (u !== null) {
        if (!matchesOAuthBaseUrlHost(p?.baseURL)) {
          ...
          return (logForDebugging(R), { headers: {}, error: R, profileError: !0 });
        }
...
async function Pe(e, t = !1, o) {
  ...
  try {
    ...
    if (v.status === 304) {
      if (!e)
        return (
          logForDebugging("Remote settings: 304 to an unconditional request"),
          {
            success: !1,
    ...
    if (v.status === 204 || v.status === 404)
      return (
        logForDebugging(`Remote settings: No settings found (${v.status})`),
        { success: !0, settings: {}, checksum: void 0, consentIdentity: u }
      );
    let P = me().safeParse(v.data);
    if (!P.success)
      return (
        logForDebugging(`Remote settings: Invalid response format - ${P.error.message}`),
        {
          success: !1,
    ...
    switch (w) {
      case "auth": {
        ...
        if (_ === 401 && a && t) setSettingsBearerRejected(!0);
        return p(P);
      }
      case "timeout":
        return p({
...
async function he(e, t, o) {
  ...
  if (isHoverRestEnabled() && t !== void 0 && !getRemoteSettingsPathOverride()) {
    ...
    if (!u.ok) {
      logForDebugging(`Remote settings: Failed to save - ${describeStorageError(u.error)}`);
      return;
    }
    if (
      (logForDebugging("Remote settings: Saved via storage backend"),
      ...
      if (
        (logForDebugging(
          p.ok
            ...
            : `Remote settings: Failed to save helper consent - ${describeStorageError(p.error)}`,
        ),
        p.ok)
      )
  ...
  try {
    ...
    logForDebugging(`Remote settings: Saved to ${u}`);
  } catch (u) {
    logForDebugging(
  ...
  try {
    ...
    logForDebugging(`Remote settings: Failed to save helper consent - ${l(u)}`);
  }
}
async function clearRemoteManagedSettings(e) {
  (stopRemoteSettingsPoller(), resetRemoteSettingsSyncCache(), invalidateAllSettings(), O().detachBarrier()?.());
  let t =
  ...
  if (t !== void 0) {
    logForDebugging(`Remote settings: Failed to remove helper consent - ${t}`);
    return;
  }
...
async function ct(e) {
  ...
  if (!p.signedCacheShadowChecked) {
    if (((p.signedCacheShadowChecked = !0), a !== void 0))
      runSignedCacheShadowCheck("managed-settings", getSettingsPath(), a, getStoredOauthAccountInfo);
  }
  try {
    let d = await it(a, e);
    ...
    if (!d.success) {
      ...
      if (o)
        return (
          logForDebugging(`Remote settings: Using stale cache after fetch failure (${H(_)})`),
          setSessionCache(o),
          { settings: o, fetchSucceeded: !1, failure: _ }
        );
      return (
        logForDebugging(`Remote settings: Fetch failed (${H(_)}) and no cached settings`),
...
async function initializeRemoteManagedSettings(e, t, o) {
  ye();
  let r = O();
...
async function tryRefreshRemoteManagedSettings(e, t, o) {
  return (await refreshRemoteManagedSettings(e, t, o)).fetchSucceeded;
}
var dt = 5000;
async function awaitRemoteSettingsRefreshAfterLogin(e, t, o) {
  let r,
    a = new Promise((p) => {
      r = ne(() => p("consent_pending"));
    }),
    u = tryRefreshRemoteManagedSettings(e, t, o).then(
...
async function refreshRemoteManagedSettings(e, t, o) {
  (stopRemoteSettingsPoller(), resetRemoteSettingsSyncCache());
  ...
  if ((logForDebugging("Remote settings: Refreshed after auth change"), getSyncCacheResetEpoch() !== r))
    return (
      logForDebugging(
...
async function q() {
  try {
    let [
      ...
      import("../../02-功能模块/模型接入-Bedrock-Vertex/apply-3p-default-fallbacks.js"),
    ]);
    if (o()) e();
    else {
      t();
...
async function validateRemoteManagedSettingsLoad(e) {
  try {
...
async function lt(e, t, o) {
  ...
  try {
    ...
    if ((d ? jsonStringify(d) : null) !== a)
      (logForDebugging("Remote settings: Changed during background poll"), N());
  } catch {}
}
function Fe(e, t, o) {
  let r = O();
  ...
  if (!isRemoteSettingsEligible()) return;
  let u = createIntervalPoller(() => void lt(e, t, o), Xe, { unref: !0 });
  (r.startPoller(u), registerCleanup(u));
}
function stopRemoteSettingsPoller() {
  O().stopPoller();
}
export {
  registerManagedSettingsConsentRequester,
  acquireConsentHandoffHold,
  isConsentHandoffHeld,
  isConsentHandoffRevealActive,
  subscribeConsentHandoffHolds,
  setConsentNeededRelease,
  markNoConsentSurface,
  startRemoteSettingsLoadBarrier,
  isRemoteSettingsLoadEligible,
  awaitRemoteSettingsLoaded,
  awaitRemoteSettingsFetchSettled,
  isRemoteSettingsFetchPending,
  shouldAwaitRemoteSettingsConfirmation,
  clearRemoteManagedSettings,
  initializeRemoteManagedSettings,
  tryRefreshRemoteManagedSettings,
  awaitRemoteSettingsRefreshAfterLogin,
  refreshRemoteManagedSettings,
  validateRemoteManagedSettingsLoad,
