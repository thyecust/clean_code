...
import { getEffectiveAutoContinueAtUsageLimit } from "../../02-功能模块/AppState-状态管理/AppState-状态管理.wyzjbwp5.js";
...
function formatNotificationChannelLabel(l) {
  switch (l) {
    case "terminal_bell":
      return "bell";
...
function getSecuritySensitiveSettings() {
  return {
    ...!1,
...
function Pe() {
  ...
  return [
    "default",
    ...MODEL_ALIASES.filter(
      (r) => (l || !r.includes("fable")) && !isOpus1mModelUnavailable(r) && !isSonnet1mModelUnavailable(r) && isModelAllowed(r),
    ),
  ];
}
var Ie = new Map([
...
var B = () => {},
  noopConfigSetters = { setGlobalConfig: B, setSettingsData: B, setChanges: B };
function Fe(l) {
...
function createSettingsViewModel(l) {
  let {
    globalConfig: r,
    settingsData: f,
    themeSetting: D,
  ...
  async function z(e, o) {
    ...
    if (C) {
      ...
      if (hasPreModelSwitchHooks(T) || sessionTaskQueueStore.of(T).pending > 0) {
        ...
        if (P.messages.length > 0)
          c = ` \xB7 ${P.messages.map(toSingleLineDisplayText).join(" \xB7 ")}`;
      }
      recordModelSwitchIfChanged(T, N(), e, "command");
    }
    if (s?.fromUltracode) unpinLaunchEffortLevels(v);
    else if (s !== void 0) applyEffortLevelChange(s.level, parseModelOrDefault(e), void 0, v);
    ...
    (A(
      (T) => (
        ...
        (_ = isFastModeEnabled() ? resolveFastModeForModel(e, T.fastMode) : b),
        {
          ...
          ...(s !== void 0 && {
            sessionEffort: createEffortLevel(s.level),
    ...
    let O = formatFastModeChangeNote(b, _, e);
    w((T) => {
      let N =
  ...
  let Z = [
      ...
      ? [
          {
            ...
            async onChange(e) {
              ...
              if (o?.error) {
                let t = getEffectiveAutoContinueAtUsageLimit();
                return (
            ...
            async onChange(e) {
              ...
              if (e && C && (hasPreModelSwitchHooks(C.session) || sessionTaskQueueStore.of(C.session).pending > 0)) {
                ...
                return enqueueSessionTask(t, async () => {
                  ...
                  let b = await resolvePreModelSwitchDecision(t, s, c, "command");
                  if (d !== C.latestFastPick.current) return;
                  ...
                  return o(
                    ...
                    !0,
                    b.messages.length > 0
                      ? ` \xB7 ${b.messages.map(toSingleLineDisplayText).join(" \xB7 ")}`
                      : "",
                  );
                });
              }
      ...
      ? [
          {
            id: "notifChannel",
            label: "Notifications",
            value: formatNotificationChannelLabel(r.preferredNotifChannel),
            type: "managedEnum",
    ...
    },
    {
      ...
      onChange(e) {
        ...
        if (isFableModelBlockedByUsageCredits(o))
          return (
...
export { formatNotificationChannelLabel, getSecuritySensitiveSettings, getEffectiveConfig, noopConfigSetters, createSettingsViewModel };
