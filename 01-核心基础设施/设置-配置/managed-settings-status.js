...
// Version: 2.1.263
import { getEligibilityMemo, getIneligibleReason, getLastLoadStatus } from "./设置-配置.aqbb35ee.js";
function getManagedSettingsStatus() {
...
function isManagedSettingsFetchInProgress() {
...
function formatManagedSettingsError(e, t) {
  switch (e) {
...
function shouldReportManagedSettingsStatus(e) {
  switch (e.state) {
    case "failed":
    case "stale_cache":
      return !0;
...
function formatManagedSettingsStatus(e) {
  switch (e.state) {
    case "ok":
      return e.hasSettings ? "loaded" : "none configured for this organization";
    case "stale_cache":
      return `fetch failed \u2014 using stale cache (${formatManagedSettingsError(e.failure.errorKind, e.failure.httpStatus)})${e.transportEnvWithheld ? "; proxy/CA/provider env withheld until a fetch succeeds" : ""}`;
    case "failed":
      return `fetch failed \u2014 no policy applied (${formatManagedSettingsError(e.failure.errorKind, e.failure.httpStatus)})`;
    case "ineligible":
      return `not fetched \u2014 ${r(e.reason)}`;
  }
}
export { getManagedSettingsStatus, isManagedSettingsFetchInProgress, formatManagedSettingsError, shouldReportManagedSettingsStatus, formatManagedSettingsStatus };
