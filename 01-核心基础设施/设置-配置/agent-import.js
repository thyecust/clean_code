...
function Ze(e) {
  let t = e.unmappable.filter((o) => o.scope === "user"),
    ...
    a = [Xe(e.sourceId)];
  if (t.length > 0) {
    let o = t.map((u) => `- **${sanitizeImportField(u.label)}** \u2014 ${u.reason}`).join(`
`);
...
async function writeImportFallbackSkill(e, t) {
...
async function scanImportSources(e) {
  ...
  for (let u of r) {
    ...
    try {
      ...
      (logForDebugging(`[agentImport] ${u.id} scan failed: ${l(p)}`, { level: "error" }),
        o.push(
...
export { sanitizeImportField, sanitizeImportMessage, writeImportFallbackSkill, scanImportSources };
