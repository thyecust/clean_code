...
function findSameFileDuplicates(e) {
...
function parseTaggedPathListing(e) {
  return e
    .split("\x00")
    .filter((r) => r.length > 2 && r[1] === " ")
    .map((r) => ({ tag: r[0] ?? "", path: r.slice(2) }));
}
async function readFileWithDigests(e, r, s, i = null) {
...
async function openVerifiedFile(e, r, s) {
  ...
  let o = await r.open(s, READ_ONLY_NONBLOCK_FLAGS);
  try {
...
async function writeFileAtomically(e, r, s, i, o, t, c = SYNCED_FILE_WRITE_MODE) {
  let f = le(e, s, i);
...
function ve() {
  ...
  try {
    ...
    ((H = {
      ...
      logForDebugging("dirSync anchor: libSystem *at loaded through bun:ffi"));
  } catch (e) {
    (logForDebugging(`dirSync anchor: bun:ffi unavailable (${l(e)})`, { level: "warn" }),
      (H = null));
  }
  return H;
}
...
async function openTreeAnchor(e, r) {
...
function createFileSystemHost(e = {}) {
  return { fs: Ke, path: xe, platform: () => getCurrentPlatform(), ...e };
}
export { DEFAULT_MAX_SYNC_FILES, MAX_SYNC_UPLOAD_BYTES, isSafePortablePath, getPathIdentity, findSameFileDuplicates, parseTaggedPathListing, readFileWithDigests, READ_ONLY_NONBLOCK_FLAGS, openVerifiedFile, writeFileAtomically, openTreeAnchor, createFileSystemHost };
