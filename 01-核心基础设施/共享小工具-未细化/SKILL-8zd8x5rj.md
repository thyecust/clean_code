...
system supplies a small set of _parameters_ (its ramps, a categorical order, a
...
1. **Pick the form.** What is the data's job - magnitude, identity, polarity, a
   ...
   the answer is _not a chart_ (a stat tile or hero number). -> `references/choosing-a-form.md`
...
5. **Add the hover layer - by default.** An HTML/SVG chart _is_ interactive; ship
...
- **One axis.** Never a dual-axis chart (two y-scales). Two measures of different
  scale -> two charts, small multiples, or indexed to a common base. _(This is the
  #1 chart mistake - see anti-patterns.)_
