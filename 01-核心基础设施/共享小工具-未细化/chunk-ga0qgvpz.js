...
function mat(e) {
  switch (e.state) {
    ...
    case "stale_cache":
      return `fetch failed \u2014 using stale cache (${kOt(e.failure.errorKind, e.failure.httpStatus)})${e.transportEnvWithheld ? "; proxy/CA/provider env withheld until a fetch succeeds" : ""}`;
    case "failed":
      return `fetch failed \u2014 no policy applied (${kOt(e.failure.errorKind, e.failure.httpStatus)})`;
    case "ineligible":
      return `not fetched \u2014 ${r(e.reason)}`;
  }
}
export { Sle, MUn, kOt, NUn, mat };
