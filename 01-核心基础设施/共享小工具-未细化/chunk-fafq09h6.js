// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.263
import { uE } from "../ANSI-样式-布局原语/chunk-k8hr56nm.js";
import { Ze } from "./chunk-eebsvd7r.js";
import { Zr } from "./chunk-jhstj6d7.js";
import { Y0 } from "./chunk-ff1hq6qq.js";
import { kh } from "./chunk-tw8akhx1.js";
import { re, V, d, F } from "../../00-第三方库/_未识别/React运行时-JSX/React运行时-JSX.j03jpdbn.js";
F();
function is(i, r, e = !0) {
  let { handleInterrupt: n, handleExit: t, exitState: o } = c(r, i),
    a = V(() => ({ "app:interrupt": n, "app:exit": t }), [n, t]);
  return (Ze(a, { context: "Global", isActive: e }), o);
}
function Hen(i, r, e = !0) {
  let { handleInterrupt: n, handleExit: t, exitState: o } = c(r, i);
  return {
    entries: V(
      () =>
        e
          ? [
              { action: "app:interrupt", run: n },
              { action: "app:exit", run: t },
            ]
          : [],
      [e, n, t],
    ),
    exitState: o,
  };
}
function c(i, r) {
  let { exit: e } = uE(),
    [n, t] = d({ pending: !1, keyName: null }),
    o = V(() => r ?? e, [r, e]),
    a = kh(),
    l = Zr("app:interrupt", "Global", "Ctrl-C"),
    p = Zr("app:exit", "Global", "Ctrl-D"),
    m = a && l ? l : "Ctrl-C",
    y = a && p ? p : "Ctrl-D",
    u = Y0((s) => t({ pending: s, keyName: m }), o),
    x = Y0((s) => t({ pending: s, keyName: y }), o),
    b = re(() => {
      if (i?.()) return;
      u();
    }, [u, i]),
    C = re(() => {
      x();
    }, [x]);
  return { handleInterrupt: b, handleExit: C, exitState: n };
}
export { is, Hen };
