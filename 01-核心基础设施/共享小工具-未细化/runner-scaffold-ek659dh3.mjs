...
function usage() {
  console.error(
    "usage: node run-eval.mjs --flow DIR --variant ID [--model ID] [--reps N] [--concurrency N] [--timeout-s N (0 = no ceiling)] [--approve-harness]",
  );
}
...
async function main() {
  ...
  if (existsSync(statePath)) {
    try {
      ...
      console.error(
        `${statePath} exists but is not valid JSON (${e?.message || e}) - fix it before spending a pass`,
      );
      process.exit(2);
    }
  }
  ...
  progress();
  console.error(
    `[${args.variant}] done - ${ok} ok, ${fail} failed -> ${resultsPath}`,
  );
