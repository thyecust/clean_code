...
Good: Status tokens only when the color _means_ good/bad; categorical when it's identity.
...
Good: A legend is always present for >= 2 series; direct-label _selectively_ (the endpoint, the extreme, the one series that matters) and let the axis + tooltip carry the rest.
