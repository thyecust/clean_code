...
function coerceScores(g) {
  if (g && typeof g === "object" && !Array.isArray(g)) {
    ...
    for (const [k, v] of Object.entries(g))
      if (isNum(v) || typeof v === "boolean") out[k] = +v;
    return out;
  }
  ...
  readText(p)
    ...
    .forEach((line, i) => {
      ...
      try {
        ...
        warnings.push(
          dir + "/results.jsonl:" + (i + 1) + ": malformed JSON, skipped",
        );
        return;
      }
      ...
      if (!pid) {
        warnings.push(
          dir + "/results.jsonl:" + (i + 1) + ": no prompt_id, skipped",
        );
        return;
      }
      (byId[pid] ||= []).push(r);
...
function build(flowArg) {
  ...
  if (!variants.length)
    throw new Error(
      "no variant directories (baseline/, v1/, ...) under " + flow,
    );
  const warnings = [];
  ...
  for (const v of variants)
    rows[v] = loadRows(join(flow, v, "results.jsonl"), warnings, v);
  ...
  for (let i = variants.length - 1; i > 0; i--) {
    ...
    warnings.push(
      v +
        ": zero result rows - dropping (run not started or results.jsonl missing/empty)",
    );
    variants.splice(i, 1);
  }
  ...
  const declared = (Array.isArray(metricsCfg) ? metricsCfg : [])
    .map((m) =>
      typeof m === "string" ? { id: m } : { id: m?.id, kind: m?.kind },
    )
    .filter((m) => m.id);
  ...
  const okReps = (reps) =>
    reps.filter((r) => r.status == null || r.status === "ok");
  ...
  for (const c of cases)
    tsv.push(
      [
        tsvCell(c.id),
        tsvCell(c.split || "all"),
        ...variants.map((v) => fmt(c.per[v].mean)),
      ].join("\t"),
    );
      ...
      const cells = variants
        .map((v) => {
          ...
          const links = p.traces
            .map(
              (t) => '<a href="' + esc(t.rel) + '">rep' + esc(t.rep) + "</a>",
            )
            .join(" ");
          return (
            '<td class="num" data-v="' +
            ...
            fmt(p.mean) +
            (p.truncated
              ? ' <span class="pill warn">' + p.truncated + " truncated</span>"
              : "") +
        ...
        .join("");
      const prompt =
        c.prompt.length > 600 ? c.prompt.slice(0, 600) + "..." : c.prompt;
      return (
        '<tr><td class="id">' +
        esc(c.id) +
        "</td>" +
...
if (!arg || arg === "-h" || arg === "--help") {
  console.error(
    "usage: node build-report-lite.mjs <flow-dir>   (e.g. .claude/hillclimb/<flow>/)",
  );
