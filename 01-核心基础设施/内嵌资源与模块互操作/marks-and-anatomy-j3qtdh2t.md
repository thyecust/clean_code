...
the marks to _supplement_ it. **A single series needs no legend box**: there is only
...
- **Label selectively - never a number on every point.** A value beside every dot or
  ...
  Direct labels work _because_ they are sparing - flood the chart and they stop working.
...
- **A label that won't fit doesn't get clipped - measure first.** Only place a label
  _inside_ a bar or stacked segment when the rendered text fits with comfortable
  ...
  _interior_ stacked segment (which has no free end),
...
- **Text never wears the data color.** Marks - bars, lines, dots, area fills - carry
  ...
  as text on the surface. Identity comes from the colored mark _beside_ the text - a
  dot, a short line-key, a swatch - never from coloring the text itself. A label set
  _inside_ a colored fill (a stacked segment, a map tile) is the one exception: pick
...
texture is _ordered_ (rotation steps with magnitude; arm angle carries the diverging
