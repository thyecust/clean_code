...
var y = commonJS(function (k, I) {
  ...
  function E(r, n) {
    ...
    if (
      /^screen|^xterm|^vt100|^vt220|^rxvt|color|ansi|cygwin|linux/i.test(t.TERM)
    )
      return 1;
...
function uee(r) {
  ...
  if ("FORCE_HYPERLINK" in (r?.env ?? process.env))
    return r?.stdoutSupported ?? m.default.supportsHyperlink(process.stdout);
  return;
}
function Tf(r) {
  let n = uee(r);
  ...
  if (r?.stdoutSupported ?? m.default.supportsHyperlink(process.stdout))
    return !0;
  ...
  if (p === "tmux") {
    let [a, f] = (e.TERM_PROGRAM_VERSION ?? "").split("."),
      s = parseInt(a ?? "", 10),
      L = parseInt(f ?? "", 10);
