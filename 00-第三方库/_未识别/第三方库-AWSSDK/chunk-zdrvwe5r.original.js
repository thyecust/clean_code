...
import { isArrayBufferModule, utilUtf8Module } from "../../../02-功能模块/模型接入-Bedrock-Vertex/chunk-p991cddr.js";
...
var lhe = commonJS(function (Re) {
  var f = hexEncodingModule(),
    ...
    re = async (e, t, r, s, n) => {
      ...
      D.push(o);
      while (D.length > te) delete y[D.shift()];
    ...
    formatHeaderValue(e) {
      switch (e.type) {
        ...
        case "timestamp":
          ...
          return (
            (A[0] = 8),
            A.set(C.fromNumber(e.value.valueOf()).bytes, 1),
            A
          );
        case "uuid":
          if (!ge.test(e.value))
          ...
          return (
            (u[0] = 9),
            u.set(f.fromHex(e.value.replace(/\-/g, "")), 1),
            u
          );
      }
    }
  ...
  class C {
    ...
    static fromNumber(e) {
      ...
      for (
        let r = 7, s = Math.abs(Math.round(e));
        r > -1 && s > 0;
        r--, s /= 256
      )
        t[r] = s;
...
`);
    ...
    async signMessage(
      e,
      { signingDate: t = new Date(), signingRegion: r, signingService: s },
    ) {
